...
@details Verifies executable paths, argument vectors, project-root append
  behavior, and CODEX_HOME environment setup across cli-* and editor commands.
...
def test_cli_codex_sets_codex_home_and_executes_expected_command(monkeypatch):
    """
    @brief Validate cli-codex execution contract.
    @details Stubs project-root resolution and exec boundary, then validates
      CODEX_HOME assignment and command vector.
    @param monkeypatch {pytest.MonkeyPatch} Runtime patch helper.
    @return {None} Assertions only.
    @satisfies TST-005, REQ-014
    """
