#!/usr/bin/env python3
"""@brief Video-to-H.265 ffmpeg command module.

@details Provides CLI contract for `video2h265`, validates required positional
input, derives output path by appending `.mp4` suffix to input path token, and
executes deterministic ffmpeg argv for H.265 + AAC transcoding.
@satisfies PRJ-003, DES-008, REQ-097, REQ-098
"""
...
DESCRIPTION = "Convert a video to H.265/AAC MP4 using ffmpeg."
...
def run(args: list[str]) -> int:
    ...
    if len(args) != 1 or args[0].startswith("-"):
        print_error("Usage: video2h265 <input-video>")
        return 1
