...
DESCRIPTION = "Tile PDF to A4 pages at original A1 size using plakativ."


def print_help(version):
    print(f"Usage: {PROGRAM} pdf-tiler-100 <input.pdf> ({version})")
    print()
    print("pdf-tiler-100 options:")
...
def run(args):
    ...
    command vector with fixed A1 source size and A4 tiling parameters, then
    executes it via `subprocess.run` while inheriting stdio streams.
