...
- **REQ-065**: MUST parse optional `--post-gamma=<value>`, `--brightness=<value>`, `--contrast=<value>`, `--saturation=<value>`, `--jpg-compression=<0..100>`, and flag `--wow`; MUST default `--jpg-compression` to `15`; and MUST disable wow when omitted.
...
- **REQ-073**: MUST execute wow stage only when `--wow` is set, after shared postprocessing and before JPG encoding, using ImageMagick over temporary lossless 16-bit-per-channel files and passing wow output to JPG conversion.
...
- **TST-011**: MUST verify REQ-055 through REQ-074 by monkeypatching RAW decode, image writes, EXIF propagation, timestamp updates, and HDR subprocess calls, passing only if backend selection, parsing, help formatting, postprocessing, TIFF merge flow, and cleanup behavior match requirements.
