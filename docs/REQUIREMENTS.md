...
- **REQ-058**: MUST execute HDR merge via `enfuse` over three generated exposure files when `--enable-enfuse` is selected, MUST persist an intermediate 16-bit TIFF, MUST use lossless TIFF compression before JPG conversion, and MUST preserve resolved `ev_zero`-centered bracket extraction semantics.
...
- **REQ-062**: MUST execute `luminance-hdr-cli` with zero-reference exposure list `-e <-ev_delta,0,+ev_delta>`, `--hdrModel`, `--hdrWeight`, `--hdrResponseCurve`, `--tmo`, `--ldrTiff 16b`, and ordered inputs `<ev_minus.tif> <ev_zero.tif> <ev_plus.tif>` writing `<merged_hdr.tif>`.
- **REQ-063**: MUST document required mutually exclusive backend selectors (`--enable-enfuse`, `--enable-luminance`), required mutually exclusive exposure selectors (`--ev`, `--auto-ev`), optional `--ev-zero`/`--auto-zero` plus `--auto-zero-pct`/`--auto-ev-pct`, luminance controls, `--tmo*`, `--gamma`, shared postprocess, `--auto-brightness`/`--ab-*`, and `--auto-adjust`/`--aa-*`.
...
- **REQ-066**: MUST apply one shared postprocessing stage on merged HDR TIFF from both backends, treating the extracted `ev_zero` image as equivalent `0EV/1.0000x` reference without additional EV-zero compensation gain, then encode JPG with complete source DNG EXIF payload preserving original EXIF Orientation when available.
...
- **REQ-071**: MUST default luminance-mode postprocess factors to `1.0` for `post-gamma`, `brightness`, `contrast`, and `saturation` when `--luminance-tmo` is neither `reinhard02` nor `mantiuk08` and no explicit postprocess overrides are provided.
...
- **REQ-073**: MUST execute auto-adjust stage only when `--auto-adjust` is set, after shared postprocessing and before JPG encoding, selecting implementation by `--auto-adjust` value where `ImageMagick` and `OpenCV` both consume one shared auto-adjust parameter set.
- **REQ-075**: MUST implement `--auto-adjust OpenCV` pipeline with explicit 16-bit-per-channel to float normalization and float to 16-bit-per-channel restoration around selective blur, adaptive level, sigmoidal contrast, HSL saturation gamma, high-pass math gray, and overlay composition stages parameterized by shared auto-adjust knobs.
- **REQ-082**: MUST parse shared auto-adjust knobs `--aa-blur-sigma`, `--aa-blur-threshold-pct`, `--aa-level-low-pct`, `--aa-level-high-pct`, `--aa-sigmoid-contrast`, `--aa-sigmoid-midpoint`, `--aa-saturation-gamma`, and `--aa-highpass-blur-sigma` in assignment or split form.
- **REQ-083**: MUST default shared auto-adjust knobs to `0.9`, `5.0`, `0.1`, `99.9`, `1.8`, `0.5`, `0.8`, and `2.0` in the declaration order of REQ-082.
- **REQ-084**: MUST validate shared auto-adjust knobs with `sigma/contrast/gamma/highpass_sigma > 0`, `threshold in [0,100]`, `level_low in [0,100]`, `level_high in [0,100]`, `level_low < level_high`, and `midpoint in [0,1]`.
...
- **REQ-088**: MUST default auto-brightness parameters to `target-grey=0.18`, fixed `max-gain=4.0`, fixed `min-gain=0.95`, fixed `correction-strength=0.35`, and fixed `max-ev-correction=0.5`, and MUST treat `target-grey` as linear BT.709 middle-grey luminance target.
...
- **REQ-090**: MUST implement `--auto-brightness` as uint16 pipeline `normalize->linearize sRGB->BT.709 luminance->p50/p98 statistics->raw_gain(target-grey/p50)->ev(log2(raw_gain))->light_ev(clamp(ev*0.35,0,+0.5))->gain(min(2^light_ev,4.0,max(0.95,0.90/max(p98,1e-7))))->highlight rolloff->inverse sRGB->uint16` using float64 processing.
...
- **REQ-079**: MUST execute static exposure pipeline `Options -> EV-Zero Resolution (manual or auto-zero) -> Fixed Multipliers -> Extraction -> Merge -> AutoAdjust -> Save` when `--ev` is selected.
...
- **REQ-081**: MUST compute adaptive EV delta after EV-zero resolution using linear preview luminance percentiles (`0.1%`, `99.9%`) and median-aware optimization, then apply `--auto-ev-pct` scaling (default `50`), quantize downward to `0.25`, and clamp to `[0.25,MAX_BRACKET]` where `MAX_BRACKET=((bits_per_color-8)/2)-abs(ev_zero)`.
...
- **REQ-094**: MUST parse optional `--ev-zero=<value>`/`--ev-zero <value>`, optional `--auto-zero[=<1|true|yes|on>]`, optional `--auto-zero-pct=<0..100>`, and optional `--auto-ev-pct=<0..100>`; MUST enforce `--ev-zero`/`--auto-zero` mutual exclusivity, keep `0.25` granularity, default percentages to `50`, and default `ev_zero` to `0.0`.
...
- **REQ-097**: MUST compute `--auto-zero` from RAW linear preview `p50` using a scene-key-preserving target derived from shadow/median/highlight preview percentiles, then apply `--auto-zero-pct` scaling (default `50`), quantize downward to `0.25`, clamp to `[-SAFE_ZERO_MAX,+SAFE_ZERO_MAX]`, and derive `MAX_BRACKET`.
