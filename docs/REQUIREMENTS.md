...
- **REQ-058**: MUST execute HDR merge via `enfuse` over three generated exposure files when `--enable-enfuse` is selected, MUST persist an intermediate 16-bit TIFF, MUST use lossless TIFF compression before JPG conversion, and MUST preserve selected `ev_zero` center without implicit re-centering to `0`.
...
- **REQ-062**: MUST execute `luminance-hdr-cli` with `-e <ev_minus,ev_zero,ev_plus>`, `--hdrModel`, `--hdrWeight`, `--hdrResponseCurve`, `--tmo`, `--ldrTiff 16b`, and ordered inputs `<ev_minus.tif> <ev_zero.tif> <ev_plus.tif>` writing `<merged_hdr.tif>` while preserving selected `ev_zero` center without implicit re-centering to `0`.
...
- **REQ-066**: MUST apply one shared postprocessing stage on merged HDR TIFF from both backends, applying `ev_zero` compensation gain `2^(ev_zero)` when `ev_zero != 0` before static gamma/brightness/contrast/saturation corrections, then encode JPG with complete source DNG EXIF payload preserving original EXIF Orientation when available.
...
- **REQ-095**: MUST export bracket exposures centered on `ev_zero`, generating EV triplet `<ev_zero-ev_delta, ev_zero, ev_zero+ev_delta>` for `ev_minus`, `ev_zero`, and `ev_plus`, and MUST keep merged HDR luminance aligned to the same center.
