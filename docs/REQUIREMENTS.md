...
- **REQ-063**: MUST document required mutually exclusive backend selectors (`--enable-enfuse`, `--enable-luminance`), required mutually exclusive exposure selectors (`--ev`, `--auto-ev`), optional `--ev-zero` and `--auto-zero`, luminance controls, generic `--tmo*`, `--gamma`, shared postprocess, `--auto-brightness`/`--ab-*`, and `--auto-adjust`/`--aa-*`.
...
- **REQ-088**: MUST default auto-brightness knobs to `clip-limit=2.0`, `tile-grid-size=8,8`, `target-mean=0.52`, `mean-tolerance=0.03`, and `initial-clip-hist-percent=1.0` when `--auto-brightness` is set and no explicit `--ab-*` overrides are provided.
...
- **REQ-090**: MUST implement `--auto-brightness` with OpenCV luminance-channel pipeline `histogram-clip autolevel -> CLAHE -> conditional gamma(mean,target,tolerance)` before static postprocess factors, applying static factors on the auto-brightness output without changing behavior when `--auto-brightness` is disabled.
