...
- **REQ-083**: MUST default shared auto-adjust knobs to `2.0`, `10.0`, `0.1`, `99.9`, `3.0`, `0.5`, `0.8`, and `2.5` in the declaration order of REQ-082.
...
- **REQ-090**: MUST implement `--auto-brightness` with OpenCV luminance-channel pipeline `histogram-clip autolevel -> CLAHE -> conditional gamma(mean,target,tolerance)` before static postprocess factors, where `target` is derived by scene-key-preserving low/mid/high classification.
- **REQ-099**: MUST NOT force low-key or high-key scenes toward a fixed global mean target; auto-brightness gamma targeting MUST preserve the input scene-key class while applying luminance correction.
