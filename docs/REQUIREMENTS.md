...
- **REQ-090**: MUST implement `--auto-brightness` as uint16 pipeline `normalize->linearize sRGB->BT.709 luminance->p50/p98 statistics->gain(min(target-grey/p50,4.0,0.90/max(p98,1e-7)))->highlight rolloff->inverse sRGB->uint16` using float64 processing.
