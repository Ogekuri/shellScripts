...
- **REQ-075**: MUST implement `magic_retouch` as deterministic adaptive OpenCV processing on RGB float payloads with ordered stages: conditional denoise, luminance-aware gamma, optional local contrast enhancement, conditional vibrance, and conditional edge-masked sharpening; when `--magic-denoise-threshold=0`, `--magic-gamma-bias=0`, `--magic-vibrance-strength=0`, or `--magic-sharpen-strength=0`, the corresponding stage computation MUST be bypassed.
