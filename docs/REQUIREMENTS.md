...
- **REQ-043**: MUST copy `~/.codex/auth.json` to `<project-root>/.codex/auth.json` before `codex` executes `codex --yolo`, replacing any existing file or symlink at the project auth path.
- **REQ-044**: MUST copy `<project-root>/.codex/auth.json` to `~/.codex/auth.json` after `codex --yolo` terminates, replacing any existing file or symlink at the user-home auth path.
...
- **TST-005**: MUST verify REQ-014 through REQ-021 and REQ-043 through REQ-044 by monkeypatching `subprocess.run` and filesystem/environment state, passing only if executable args, `CODEX_HOME`, codex auth file synchronization behavior, and propagated return codes match requirements.
...
| REQ-014, REQ-043, REQ-044, REQ-064 | `src/shell_scripts/commands/codex.py` | `run` | Sets `CODEX_HOME=<project>/.codex`; copies `~/.codex/auth.json` into `<project>/.codex/auth.json` before launch and copies back after process termination, replacing existing file or symlink targets; executes `codex --yolo` via `subprocess.run` with inherited stdio and blocking wait. |
