...
# dng2hdr2jpg.py | Python | 4230L | 138 symbols | 19 imports | 93 comments
...
- var `DEFAULT_AB_P98_HIGHLIGHT_MAX = 0.90` (L50)
- var `DEFAULT_LUMINANCE_HDR_MODEL = "debevec"` (L51)
- var `DEFAULT_LUMINANCE_HDR_WEIGHT = "flat"` (L52)
- var `DEFAULT_LUMINANCE_HDR_RESPONSE_CURVE = "srgb"` (L53)
- var `DEFAULT_LUMINANCE_TMO = "mantiuk08"` (L54)
- var `DEFAULT_REINHARD02_BRIGHTNESS = 1.25` (L55)
- var `DEFAULT_REINHARD02_CONTRAST = 0.85` (L56)
- var `DEFAULT_REINHARD02_SATURATION = 0.55` (L57)
- var `DEFAULT_MANTIUK08_CONTRAST = 1.2` (L58)
- var `EV_STEP = 0.25` (L59)
- var `MIN_SUPPORTED_BITS_PER_COLOR = 9` (L60)
- var `DEFAULT_DNG_BITS_PER_COLOR = 14` (L61)
- var `SUPPORTED_EV_VALUES = tuple(` (L62)
- var `AUTO_EV_LOW_PERCENTILE = 0.1` (L68)
- var `AUTO_EV_HIGH_PERCENTILE = 99.9` (L69)
- var `AUTO_EV_MEDIAN_PERCENTILE = 50.0` (L70)
- var `AUTO_EV_TARGET_SHADOW = 0.05` (L71)
- var `AUTO_EV_TARGET_HIGHLIGHT = 0.90` (L72)
- var `AUTO_EV_MEDIAN_TARGET = 0.5` (L73)
- var `AUTO_ZERO_SCENE_KEY_LOW_THRESHOLD = 0.35` (L74)
- var `AUTO_ZERO_SCENE_KEY_HIGH_THRESHOLD = 0.65` (L75)
- var `AUTO_ZERO_TARGET_LOW_KEY = 0.35` (L76)
- var `AUTO_ZERO_TARGET_HIGH_KEY = 0.65` (L77)
### class `class AutoAdjustOptions` `@dataclass(frozen=True)` (L251-278)
...
### class `class AutoBrightnessOptions` `@dataclass(frozen=True)` (L280-292)
- @brief Hold `--auto-brightness` knob values.
- @details Encapsulates validated auto-brightness parameters for the BT.709 linear-sRGB luminance pipeline with global gain and highlight rolloff.
- @param target_grey {float} Linear BT.709 middle-grey luminance target in `(0, 1)`.
- @return {None} Immutable dataclass container.
- @satisfies REQ-065, REQ-088, REQ-089, REQ-090

### class `class PostprocessOptions` `@dataclass(frozen=True)` (L294-324)
...
### class `class LuminanceOptions` `@dataclass(frozen=True)` (L326-346)
...
### class `class AutoEvInputs` `@dataclass(frozen=True)` (L348-375)
...
### fn `def _print_box_table(headers, rows, header_rows=())` `priv` (L376-412)
...
### fn `def _border(left, middle, right)` `priv` (L396-398)
- @brief Print one Unicode box-drawing table.
...
### fn `def _line(values)` `priv` (L399-402)

### fn `def _build_two_line_operator_rows(operator_entries)` `priv` (L413-429)
...
### fn `def print_help(version)` (L430-613)
...
### fn `def _calculate_max_ev_from_bits(bits_per_color)` `priv` (L614-632)
...
### fn `def _calculate_safe_ev_zero_max(base_max_ev)` `priv` (L633-645)
...
### fn `def _derive_supported_ev_zero_values(base_max_ev)` `priv` (L646-662)
...
### fn `def _derive_supported_ev_values(bits_per_color, ev_zero=0.0)` `priv` (L663-691)
...
### fn `def _detect_dng_bits_per_color(raw_handle)` `priv` (L692-737)
...
### fn `def _is_ev_value_on_supported_step(ev_value)` `priv` (L738-751)
...
### fn `def _parse_ev_option(ev_raw)` `priv` (L752-783)
...
### fn `def _parse_ev_zero_option(ev_zero_raw)` `priv` (L784-814)
...
### fn `def _parse_auto_ev_option(auto_ev_raw)` `priv` (L815-832)
...
### fn `def _parse_auto_zero_option(auto_zero_raw)` `priv` (L833-850)
...
### fn `def _parse_percentage_option(option_name, option_raw)` `priv` (L851-873)
...
### fn `def _parse_auto_brightness_option(auto_brightness_raw)` `priv` (L874-891)
...
### fn `def _clamp_ev_to_supported(ev_candidate, ev_values)` `priv` (L892-905)
...
### fn `def _quantize_ev_to_supported(ev_value, ev_values)` `priv` (L906-927)
...
### fn `def _quantize_ev_toward_zero_step(ev_value, step=EV_STEP)` `priv` (L928-949)
...
### fn `def _apply_auto_percentage_scaling(ev_value, percentage)` `priv` (L950-964)
...
### fn `def _extract_normalized_preview_luminance_stats(raw_handle)` `priv` (L965-1024)
...
### fn `def _percentile(percentile_value)` `priv` (L999-1009)
- @brief Extract normalized preview luminance percentiles from RAW handle.
...
### fn `def _coerce_positive_luminance(value, fallback)` `priv` (L1025-1044)
...
### fn `def _derive_scene_key_preserving_median_target(p_median)` `priv` (L1045-1063)
...
### fn `def _optimize_auto_zero(auto_ev_inputs)` `priv` (L1064-1087)
...
### fn `def _optimize_adaptive_ev_delta(auto_ev_inputs)` `priv` (L1088-1117)
...
### fn `def _compute_auto_ev_value_from_stats(` `priv` (L1118-1123)

### fn `def _compute_auto_ev_value(raw_handle, supported_ev_values=None, ev_zero=0.0)` `priv` (L1151-1178)
...
### fn `def _resolve_ev_zero(` `priv` (L1179-1186)

### fn `def _resolve_ev_value(` `priv` (L1237-1244)
...
### fn `def _parse_luminance_text_option(option_name, option_raw)` `priv` (L1297-1317)
...
### fn `def _parse_gamma_option(gamma_raw)` `priv` (L1318-1354)
...
### fn `def _parse_positive_float_option(option_name, option_raw)` `priv` (L1355-1378)
...
### fn `def _parse_tmo_passthrough_value(option_name, option_raw)` `priv` (L1379-1395)
...
### fn `def _parse_jpg_compression_option(compression_raw)` `priv` (L1396-1418)
...
### fn `def _parse_float_exclusive_range_option(option_name, option_raw, min_value, max_value)` `priv` (L1419-1443)
...
### fn `def _parse_non_negative_float_option(option_name, option_raw)` `priv` (L1444-1466)
...
### fn `def _parse_float_in_range_option(option_name, option_raw, min_value, max_value)` `priv` (L1467-1492)
...
### fn `def _parse_auto_brightness_options(auto_brightness_raw_values)` `priv` (L1493-1516)
...
### fn `def _parse_auto_adjust_options(auto_adjust_raw_values)` `priv` (L1517-1623)
...
### fn `def _parse_auto_adjust_mode_option(auto_adjust_raw)` `priv` (L1624-1647)
...
### fn `def _resolve_default_postprocess(enable_luminance, luminance_tmo)` `priv` (L1648-1690)
...
### fn `def _parse_run_options(args)` `priv` (L1691-1890)
...
### fn `def _load_image_dependencies()` `priv` (L2326-2364)
...
### fn `def _parse_exif_datetime_to_timestamp(datetime_raw)` `priv` (L2365-2395)
...
### fn `def _extract_dng_exif_payload_and_timestamp(pil_image_module, input_dng)` `priv` (L2396-2453)
...
### fn `def _resolve_thumbnail_transpose_map(pil_image_module)` `priv` (L2454-2485)
...
### fn `def _apply_orientation_transform(pil_image_module, pil_image, source_orientation)` `priv` (L2486-2508)
...
### fn `def _build_oriented_thumbnail_jpeg_bytes(` `priv` (L2509-2510)

### fn `def _coerce_exif_int_like_value(raw_value)` `priv` (L2539-2581)
...
### fn `def _normalize_ifd_integer_like_values_for_piexif_dump(piexif_module, exif_dict)` `priv` (L2582-2715)
...
### fn `def _refresh_output_jpg_exif_thumbnail_after_save(` `priv` (L2716-2721)

### fn `def _set_output_file_timestamps(output_jpg, exif_timestamp)` `priv` (L2770-2784)
...
### fn `def _sync_output_file_timestamps_from_exif(output_jpg, exif_timestamp)` `priv` (L2785-2801)
...
### fn `def _build_exposure_multipliers(ev_value, ev_zero=0.0)` `priv` (L2802-2820)
...
### fn `def _write_bracket_images(` `priv` (L2821-2822)

### fn `def _order_bracket_paths(bracket_paths)` `priv` (L2859-2884)
...
### fn `def _run_enfuse(bracket_paths, merged_tiff)` `priv` (L2885-2905)
...
### fn `def _run_luminance_hdr_cli(` `priv` (L2906-2907)

### fn `def _convert_compression_to_quality(jpg_compression)` `priv` (L2959-2971)
...
### fn `def _resolve_imagemagick_command()` `priv` (L2972-2989)
...
### fn `def _resolve_auto_adjust_opencv_dependencies()` `priv` (L2990-3014)
...
### fn `def _to_uint8_image_array(np_module, image_data)` `priv` (L3015-3061)
...
### fn `def _to_uint16_image_array(np_module, image_data)` `priv` (L3062-3106)
...
### fn `def _to_linear_srgb(np_module, image_srgb)` `priv` (L3107-3124)
...
### fn `def _from_linear_srgb(np_module, image_linear)` `priv` (L3125-3142)
...
### fn `def _compute_bt709_luminance(np_module, linear_rgb)` `priv` (L3143-3160)
...
### fn `def _apply_highlight_rolloff(np_module, linear_rgb)` `priv` (L3161-3179)
...
### fn `def _apply_auto_brightness_rgb_uint8(` `priv` (L3180-3181)

### fn `def _apply_validated_auto_adjust_pipeline(` `priv` (L3229-3230)
...
### fn `def _clamp01(np_module, values)` `priv` (L3311-3324)
...
### fn `def _gaussian_kernel_2d(np_module, sigma, radius=None)` `priv` (L3325-3347)
...
### fn `def _rgb_to_hsl(np_module, rgb)` `priv` (L3348-3381)
...
### fn `def _hue_to_rgb(np_module, p_values, q_values, t_values)` `priv` (L3382-3412)
...
### fn `def _hsl_to_rgb(np_module, hue, saturation, lightness)` `priv` (L3413-3453)
...
### fn `def _selective_blur_contrast_gated_vectorized(` `priv` (L3454-3455)

### fn `def _level_per_channel_adaptive(np_module, rgb, low_pct=0.1, high_pct=99.9)` `priv` (L3504-3526)
...
### fn `def _sigmoidal_contrast(np_module, rgb, contrast=3.0, midpoint=0.5)` `priv` (L3527-3551)
...
### fn `def logistic(z_values)` (L3542-3544)
- @brief Execute sigmoidal contrast stage.
...
### fn `def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=0.8)` `priv` (L3552-3569)
...
### fn `def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)` `priv` (L3570-3593)
...
### fn `def _high_pass_math_gray(cv2_module, np_module, rgb, blur_sigma=2.5)` `priv` (L3594-3617)
...
### fn `def _overlay_composite(np_module, base_rgb, overlay_gray)` `priv` (L3618-3639)
...
### fn `def _apply_validated_auto_adjust_pipeline_opencv(` `priv` (L3640-3641)

### fn `def _load_piexif_dependency()` `priv` (L3720-3737)
...
### fn `def _encode_jpg(` `priv` (L3738-3750)

### fn `def _collect_processing_errors(rawpy_module)` `priv` (L3958-3986)
...
### fn `def _is_supported_runtime_os()` `priv` (L3987-4006)
...
|`DEFAULT_AB_P98_HIGHLIGHT_MAX`|var|pub|50||
|`DEFAULT_LUMINANCE_HDR_MODEL`|var|pub|51||
|`DEFAULT_LUMINANCE_HDR_WEIGHT`|var|pub|52||
|`DEFAULT_LUMINANCE_HDR_RESPONSE_CURVE`|var|pub|53||
|`DEFAULT_LUMINANCE_TMO`|var|pub|54||
|`DEFAULT_REINHARD02_BRIGHTNESS`|var|pub|55||
|`DEFAULT_REINHARD02_CONTRAST`|var|pub|56||
|`DEFAULT_REINHARD02_SATURATION`|var|pub|57||
|`DEFAULT_MANTIUK08_CONTRAST`|var|pub|58||
|`EV_STEP`|var|pub|59||
|`MIN_SUPPORTED_BITS_PER_COLOR`|var|pub|60||
|`DEFAULT_DNG_BITS_PER_COLOR`|var|pub|61||
|`SUPPORTED_EV_VALUES`|var|pub|62||
|`AUTO_EV_LOW_PERCENTILE`|var|pub|68||
|`AUTO_EV_HIGH_PERCENTILE`|var|pub|69||
|`AUTO_EV_MEDIAN_PERCENTILE`|var|pub|70||
|`AUTO_EV_TARGET_SHADOW`|var|pub|71||
|`AUTO_EV_TARGET_HIGHLIGHT`|var|pub|72||
|`AUTO_EV_MEDIAN_TARGET`|var|pub|73||
|`AUTO_ZERO_SCENE_KEY_LOW_THRESHOLD`|var|pub|74||
|`AUTO_ZERO_SCENE_KEY_HIGH_THRESHOLD`|var|pub|75||
|`AUTO_ZERO_TARGET_LOW_KEY`|var|pub|76||
|`AUTO_ZERO_TARGET_HIGH_KEY`|var|pub|77||
|`AutoAdjustOptions`|class|pub|251-278|class AutoAdjustOptions|
|`AutoBrightnessOptions`|class|pub|280-292|class AutoBrightnessOptions|
|`PostprocessOptions`|class|pub|294-324|class PostprocessOptions|
|`LuminanceOptions`|class|pub|326-346|class LuminanceOptions|
|`AutoEvInputs`|class|pub|348-375|class AutoEvInputs|
|`_print_box_table`|fn|priv|376-412|def _print_box_table(headers, rows, header_rows=())|
|`_border`|fn|priv|396-398|def _border(left, middle, right)|
|`_line`|fn|priv|399-402|def _line(values)|
|`_build_two_line_operator_rows`|fn|priv|413-429|def _build_two_line_operator_rows(operator_entries)|
|`print_help`|fn|pub|430-613|def print_help(version)|
|`_calculate_max_ev_from_bits`|fn|priv|614-632|def _calculate_max_ev_from_bits(bits_per_color)|
|`_calculate_safe_ev_zero_max`|fn|priv|633-645|def _calculate_safe_ev_zero_max(base_max_ev)|
|`_derive_supported_ev_zero_values`|fn|priv|646-662|def _derive_supported_ev_zero_values(base_max_ev)|
|`_derive_supported_ev_values`|fn|priv|663-691|def _derive_supported_ev_values(bits_per_color, ev_zero=0.0)|
|`_detect_dng_bits_per_color`|fn|priv|692-737|def _detect_dng_bits_per_color(raw_handle)|
|`_is_ev_value_on_supported_step`|fn|priv|738-751|def _is_ev_value_on_supported_step(ev_value)|
|`_parse_ev_option`|fn|priv|752-783|def _parse_ev_option(ev_raw)|
|`_parse_ev_zero_option`|fn|priv|784-814|def _parse_ev_zero_option(ev_zero_raw)|
|`_parse_auto_ev_option`|fn|priv|815-832|def _parse_auto_ev_option(auto_ev_raw)|
|`_parse_auto_zero_option`|fn|priv|833-850|def _parse_auto_zero_option(auto_zero_raw)|
|`_parse_percentage_option`|fn|priv|851-873|def _parse_percentage_option(option_name, option_raw)|
|`_parse_auto_brightness_option`|fn|priv|874-891|def _parse_auto_brightness_option(auto_brightness_raw)|
|`_clamp_ev_to_supported`|fn|priv|892-905|def _clamp_ev_to_supported(ev_candidate, ev_values)|
|`_quantize_ev_to_supported`|fn|priv|906-927|def _quantize_ev_to_supported(ev_value, ev_values)|
|`_quantize_ev_toward_zero_step`|fn|priv|928-949|def _quantize_ev_toward_zero_step(ev_value, step=EV_STEP)|
|`_apply_auto_percentage_scaling`|fn|priv|950-964|def _apply_auto_percentage_scaling(ev_value, percentage)|
|`_extract_normalized_preview_luminance_stats`|fn|priv|965-1024|def _extract_normalized_preview_luminance_stats(raw_handle)|
|`_percentile`|fn|priv|999-1009|def _percentile(percentile_value)|
|`_coerce_positive_luminance`|fn|priv|1025-1044|def _coerce_positive_luminance(value, fallback)|
|`_derive_scene_key_preserving_median_target`|fn|priv|1045-1063|def _derive_scene_key_preserving_median_target(p_median)|
|`_optimize_auto_zero`|fn|priv|1064-1087|def _optimize_auto_zero(auto_ev_inputs)|
|`_optimize_adaptive_ev_delta`|fn|priv|1088-1117|def _optimize_adaptive_ev_delta(auto_ev_inputs)|
|`_compute_auto_ev_value_from_stats`|fn|priv|1118-1123|def _compute_auto_ev_value_from_stats(|
|`_compute_auto_ev_value`|fn|priv|1151-1178|def _compute_auto_ev_value(raw_handle, supported_ev_value...|
|`_resolve_ev_zero`|fn|priv|1179-1186|def _resolve_ev_zero(|
|`_resolve_ev_value`|fn|priv|1237-1244|def _resolve_ev_value(|
|`_parse_luminance_text_option`|fn|priv|1297-1317|def _parse_luminance_text_option(option_name, option_raw)|
|`_parse_gamma_option`|fn|priv|1318-1354|def _parse_gamma_option(gamma_raw)|
|`_parse_positive_float_option`|fn|priv|1355-1378|def _parse_positive_float_option(option_name, option_raw)|
|`_parse_tmo_passthrough_value`|fn|priv|1379-1395|def _parse_tmo_passthrough_value(option_name, option_raw)|
|`_parse_jpg_compression_option`|fn|priv|1396-1418|def _parse_jpg_compression_option(compression_raw)|
|`_parse_float_exclusive_range_option`|fn|priv|1419-1443|def _parse_float_exclusive_range_option(option_name, opti...|
|`_parse_non_negative_float_option`|fn|priv|1444-1466|def _parse_non_negative_float_option(option_name, option_...|
|`_parse_float_in_range_option`|fn|priv|1467-1492|def _parse_float_in_range_option(option_name, option_raw,...|
|`_parse_auto_brightness_options`|fn|priv|1493-1516|def _parse_auto_brightness_options(auto_brightness_raw_va...|
|`_parse_auto_adjust_options`|fn|priv|1517-1623|def _parse_auto_adjust_options(auto_adjust_raw_values)|
|`_parse_auto_adjust_mode_option`|fn|priv|1624-1647|def _parse_auto_adjust_mode_option(auto_adjust_raw)|
|`_resolve_default_postprocess`|fn|priv|1648-1690|def _resolve_default_postprocess(enable_luminance, lumina...|
|`_parse_run_options`|fn|priv|1691-1890|def _parse_run_options(args)|
|`_load_image_dependencies`|fn|priv|2326-2364|def _load_image_dependencies()|
|`_parse_exif_datetime_to_timestamp`|fn|priv|2365-2395|def _parse_exif_datetime_to_timestamp(datetime_raw)|
|`_extract_dng_exif_payload_and_timestamp`|fn|priv|2396-2453|def _extract_dng_exif_payload_and_timestamp(pil_image_mod...|
|`_resolve_thumbnail_transpose_map`|fn|priv|2454-2485|def _resolve_thumbnail_transpose_map(pil_image_module)|
|`_apply_orientation_transform`|fn|priv|2486-2508|def _apply_orientation_transform(pil_image_module, pil_im...|
|`_build_oriented_thumbnail_jpeg_bytes`|fn|priv|2509-2510|def _build_oriented_thumbnail_jpeg_bytes(|
|`_coerce_exif_int_like_value`|fn|priv|2539-2581|def _coerce_exif_int_like_value(raw_value)|
|`_normalize_ifd_integer_like_values_for_piexif_dump`|fn|priv|2582-2715|def _normalize_ifd_integer_like_values_for_piexif_dump(pi...|
|`_refresh_output_jpg_exif_thumbnail_after_save`|fn|priv|2716-2721|def _refresh_output_jpg_exif_thumbnail_after_save(|
|`_set_output_file_timestamps`|fn|priv|2770-2784|def _set_output_file_timestamps(output_jpg, exif_timestamp)|
|`_sync_output_file_timestamps_from_exif`|fn|priv|2785-2801|def _sync_output_file_timestamps_from_exif(output_jpg, ex...|
|`_build_exposure_multipliers`|fn|priv|2802-2820|def _build_exposure_multipliers(ev_value, ev_zero=0.0)|
|`_write_bracket_images`|fn|priv|2821-2822|def _write_bracket_images(|
|`_order_bracket_paths`|fn|priv|2859-2884|def _order_bracket_paths(bracket_paths)|
|`_run_enfuse`|fn|priv|2885-2905|def _run_enfuse(bracket_paths, merged_tiff)|
|`_run_luminance_hdr_cli`|fn|priv|2906-2907|def _run_luminance_hdr_cli(|
|`_convert_compression_to_quality`|fn|priv|2959-2971|def _convert_compression_to_quality(jpg_compression)|
|`_resolve_imagemagick_command`|fn|priv|2972-2989|def _resolve_imagemagick_command()|
|`_resolve_auto_adjust_opencv_dependencies`|fn|priv|2990-3014|def _resolve_auto_adjust_opencv_dependencies()|
|`_to_uint8_image_array`|fn|priv|3015-3061|def _to_uint8_image_array(np_module, image_data)|
|`_to_uint16_image_array`|fn|priv|3062-3106|def _to_uint16_image_array(np_module, image_data)|
|`_to_linear_srgb`|fn|priv|3107-3124|def _to_linear_srgb(np_module, image_srgb)|
|`_from_linear_srgb`|fn|priv|3125-3142|def _from_linear_srgb(np_module, image_linear)|
|`_compute_bt709_luminance`|fn|priv|3143-3160|def _compute_bt709_luminance(np_module, linear_rgb)|
|`_apply_highlight_rolloff`|fn|priv|3161-3179|def _apply_highlight_rolloff(np_module, linear_rgb)|
|`_apply_auto_brightness_rgb_uint8`|fn|priv|3180-3181|def _apply_auto_brightness_rgb_uint8(|
|`_apply_validated_auto_adjust_pipeline`|fn|priv|3229-3230|def _apply_validated_auto_adjust_pipeline(|
|`_clamp01`|fn|priv|3311-3324|def _clamp01(np_module, values)|
|`_gaussian_kernel_2d`|fn|priv|3325-3347|def _gaussian_kernel_2d(np_module, sigma, radius=None)|
|`_rgb_to_hsl`|fn|priv|3348-3381|def _rgb_to_hsl(np_module, rgb)|
|`_hue_to_rgb`|fn|priv|3382-3412|def _hue_to_rgb(np_module, p_values, q_values, t_values)|
|`_hsl_to_rgb`|fn|priv|3413-3453|def _hsl_to_rgb(np_module, hue, saturation, lightness)|
|`_selective_blur_contrast_gated_vectorized`|fn|priv|3454-3455|def _selective_blur_contrast_gated_vectorized(|
|`_level_per_channel_adaptive`|fn|priv|3504-3526|def _level_per_channel_adaptive(np_module, rgb, low_pct=0...|
|`_sigmoidal_contrast`|fn|priv|3527-3551|def _sigmoidal_contrast(np_module, rgb, contrast=3.0, mid...|
|`logistic`|fn|pub|3542-3544|def logistic(z_values)|
|`_vibrance_hsl_gamma`|fn|priv|3552-3569|def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=...|
|`_gaussian_blur_rgb`|fn|priv|3570-3593|def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)|
|`_high_pass_math_gray`|fn|priv|3594-3617|def _high_pass_math_gray(cv2_module, np_module, rgb, blur...|
|`_overlay_composite`|fn|priv|3618-3639|def _overlay_composite(np_module, base_rgb, overlay_gray)|
|`_apply_validated_auto_adjust_pipeline_opencv`|fn|priv|3640-3641|def _apply_validated_auto_adjust_pipeline_opencv(|
|`_load_piexif_dependency`|fn|priv|3720-3737|def _load_piexif_dependency()|
|`_encode_jpg`|fn|priv|3738-3750|def _encode_jpg(|
|`_collect_processing_errors`|fn|priv|3958-3986|def _collect_processing_errors(rawpy_module)|
|`_is_supported_runtime_os`|fn|priv|3987-4006|def _is_supported_runtime_os()|
|`run`|fn|pub|4007-4206|def run(args)|
