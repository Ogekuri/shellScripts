...
# dng2hdr2jpg.py | Python | 3096L | 99 symbols | 19 imports | 65 comments
...
- var `DEFAULT_LUMINANCE_HDR_MODEL = "debevec"` (L46)
- var `DEFAULT_LUMINANCE_HDR_WEIGHT = "flat"` (L47)
- var `DEFAULT_LUMINANCE_HDR_RESPONSE_CURVE = "srgb"` (L48)
- var `DEFAULT_LUMINANCE_TMO = "reinhard02"` (L49)
- var `DEFAULT_REINHARD02_BRIGHTNESS = 1.25` (L50)
- var `DEFAULT_REINHARD02_CONTRAST = 0.85` (L51)
- var `DEFAULT_REINHARD02_SATURATION = 0.55` (L52)
...
### class `class AutoAdjustOptions` `@dataclass(frozen=True)` (L232-259)
...
### class `class PostprocessOptions` `@dataclass(frozen=True)` (L261-285)
...
### class `class LuminanceOptions` `@dataclass(frozen=True)` (L287-307)
...
### class `class AutoEvInputs` `@dataclass(frozen=True)` (L309-336)
...
### fn `def _print_box_table(headers, rows, header_rows=())` `priv` (L337-373)
...
### fn `def _border(left, middle, right)` `priv` (L357-359)
- @brief Print one Unicode box-drawing table.
...
### fn `def _line(values)` `priv` (L360-363)

### fn `def _build_two_line_operator_rows(operator_entries)` `priv` (L374-390)
...
### fn `def print_help(version)` (L391-535)
...
### fn `def _parse_ev_option(ev_raw)` `priv` (L536-560)
...
### fn `def _parse_auto_ev_option(auto_ev_raw)` `priv` (L561-578)
...
### fn `def _clamp_ev_to_supported(ev_candidate, ev_min=AUTO_EV_MIN, ev_max=AUTO_EV_MAX)` `priv` (L579-593)
...
### fn `def _quantize_ev_to_supported(ev_value)` `priv` (L594-614)
...
### fn `def _coerce_positive_luminance(value, fallback)` `priv` (L615-634)
...
### fn `def _optimize_adaptive_ev_delta(auto_ev_inputs)` `priv` (L635-656)
...
### fn `def _compute_auto_ev_value(raw_handle)` `priv` (L657-727)
...
### fn `def _percentile(percentile_value)` `priv` (L691-701)
- @brief Compute adaptive EV selector from RAW linear preview histogram.
...
### fn `def _resolve_ev_value(raw_handle, ev_value, auto_ev_enabled)` `priv` (L728-747)
...
### fn `def _parse_luminance_text_option(option_name, option_raw)` `priv` (L748-768)
...
### fn `def _parse_gamma_option(gamma_raw)` `priv` (L769-805)
...
### fn `def _parse_positive_float_option(option_name, option_raw)` `priv` (L806-829)
...
### fn `def _parse_tmo_passthrough_value(option_name, option_raw)` `priv` (L830-846)
...
### fn `def _parse_jpg_compression_option(compression_raw)` `priv` (L847-869)
...
### fn `def _parse_float_in_range_option(option_name, option_raw, min_value, max_value)` `priv` (L870-895)
...
### fn `def _parse_auto_adjust_options(auto_adjust_raw_values)` `priv` (L896-1002)
...
### fn `def _parse_auto_adjust_mode_option(auto_adjust_raw)` `priv` (L1003-1026)
...
### fn `def _resolve_default_postprocess(enable_luminance, luminance_tmo)` `priv` (L1027-1061)
...
### fn `def _parse_run_options(args)` `priv` (L1062-1261)
- @brief Parse CLI args into input, output, and EV parameters.
- @details Supports positional file arguments, required mutually exclusive exposure selectors (`--ev=<value>`/`--ev <value>` or `--auto-ev[=<1|true|yes|on>]`), optional `--gamma=<a,b>` or `--gamma <a,b>`, optional postprocess controls, optional shared auto-adjust knobs, required backend selector (`--enable-enfuse` or `--enable-luminance`), and luminance backend controls including explicit `--tmo*` passthrough options and optional auto-adjust implementation selector (`--auto-adjust <ImageMagick|OpenCV>`); rejects unknown options and invalid arity.
...
### fn `def _load_image_dependencies()` `priv` (L1538-1576)
...
### fn `def _parse_exif_datetime_to_timestamp(datetime_raw)` `priv` (L1577-1607)
...
### fn `def _extract_dng_exif_payload_and_timestamp(pil_image_module, input_dng)` `priv` (L1608-1665)
...
### fn `def _resolve_thumbnail_transpose_map(pil_image_module)` `priv` (L1666-1697)
...
### fn `def _apply_orientation_transform(pil_image_module, pil_image, source_orientation)` `priv` (L1698-1720)
...
### fn `def _build_oriented_thumbnail_jpeg_bytes(` `priv` (L1721-1722)

### fn `def _coerce_exif_int_like_value(raw_value)` `priv` (L1751-1793)
...
### fn `def _normalize_ifd_integer_like_values_for_piexif_dump(piexif_module, exif_dict)` `priv` (L1794-1927)
...
### fn `def _refresh_output_jpg_exif_thumbnail_after_save(` `priv` (L1928-1933)

### fn `def _set_output_file_timestamps(output_jpg, exif_timestamp)` `priv` (L1982-1996)
...
### fn `def _sync_output_file_timestamps_from_exif(output_jpg, exif_timestamp)` `priv` (L1997-2013)
...
### fn `def _build_exposure_multipliers(ev_value)` `priv` (L2014-2026)
...
### fn `def _write_bracket_images(` `priv` (L2027-2028)

### fn `def _order_bracket_paths(bracket_paths)` `priv` (L2065-2090)
...
### fn `def _run_enfuse(bracket_paths, merged_tiff)` `priv` (L2091-2111)
...
### fn `def _run_luminance_hdr_cli(bracket_paths, output_hdr_tiff, ev_value, luminance_options)` `priv` (L2112-2151)
...
### fn `def _convert_compression_to_quality(jpg_compression)` `priv` (L2152-2164)
...
### fn `def _resolve_imagemagick_command()` `priv` (L2165-2182)
...
### fn `def _resolve_auto_adjust_opencv_dependencies()` `priv` (L2183-2206)
- @brief Resolve OpenCV auto-adjust runtime dependencies.
- @details Imports `cv2` and `numpy` modules required by OpenCV auto-adjust pipeline execution and returns `None` with deterministic error output when missing.
...
### fn `def _apply_validated_auto_adjust_pipeline(` `priv` (L2207-2208)

### fn `def _clamp01(np_module, values)` `priv` (L2289-2302)
...
### fn `def _gaussian_kernel_2d(np_module, sigma, radius=None)` `priv` (L2303-2325)
...
### fn `def _rgb_to_hsl(np_module, rgb)` `priv` (L2326-2359)
...
### fn `def _hue_to_rgb(np_module, p_values, q_values, t_values)` `priv` (L2360-2390)
...
### fn `def _hsl_to_rgb(np_module, hue, saturation, lightness)` `priv` (L2391-2431)
...
### fn `def _selective_blur_contrast_gated_vectorized(` `priv` (L2432-2433)

### fn `def _level_per_channel_adaptive(np_module, rgb, low_pct=0.1, high_pct=99.9)` `priv` (L2482-2504)
...
### fn `def _sigmoidal_contrast(np_module, rgb, contrast=3.0, midpoint=0.5)` `priv` (L2505-2529)
...
### fn `def logistic(z_values)` (L2520-2522)
- @brief Execute sigmoidal contrast stage.
...
### fn `def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=0.8)` `priv` (L2530-2547)
...
### fn `def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)` `priv` (L2548-2571)
...
### fn `def _high_pass_math_gray(cv2_module, np_module, rgb, blur_sigma=2.5)` `priv` (L2572-2595)
...
### fn `def _overlay_composite(np_module, base_rgb, overlay_gray)` `priv` (L2596-2617)
...
### fn `def _apply_validated_auto_adjust_pipeline_opencv(` `priv` (L2618-2619)

### fn `def _load_piexif_dependency()` `priv` (L2698-2715)
...
### fn `def _encode_jpg(` `priv` (L2716-2727)

### fn `def _collect_processing_errors(rawpy_module)` `priv` (L2883-2911)
...
### fn `def _is_supported_runtime_os()` `priv` (L2912-2931)
...
|`DEFAULT_LUMINANCE_HDR_MODEL`|var|pub|46||
|`DEFAULT_LUMINANCE_HDR_WEIGHT`|var|pub|47||
|`DEFAULT_LUMINANCE_HDR_RESPONSE_CURVE`|var|pub|48||
|`DEFAULT_LUMINANCE_TMO`|var|pub|49||
|`DEFAULT_REINHARD02_BRIGHTNESS`|var|pub|50||
|`DEFAULT_REINHARD02_CONTRAST`|var|pub|51||
|`DEFAULT_REINHARD02_SATURATION`|var|pub|52||
...
|`AutoAdjustOptions`|class|pub|232-259|class AutoAdjustOptions|
|`PostprocessOptions`|class|pub|261-285|class PostprocessOptions|
|`LuminanceOptions`|class|pub|287-307|class LuminanceOptions|
|`AutoEvInputs`|class|pub|309-336|class AutoEvInputs|
|`_print_box_table`|fn|priv|337-373|def _print_box_table(headers, rows, header_rows=())|
|`_border`|fn|priv|357-359|def _border(left, middle, right)|
|`_line`|fn|priv|360-363|def _line(values)|
|`_build_two_line_operator_rows`|fn|priv|374-390|def _build_two_line_operator_rows(operator_entries)|
|`print_help`|fn|pub|391-535|def print_help(version)|
|`_parse_ev_option`|fn|priv|536-560|def _parse_ev_option(ev_raw)|
|`_parse_auto_ev_option`|fn|priv|561-578|def _parse_auto_ev_option(auto_ev_raw)|
|`_clamp_ev_to_supported`|fn|priv|579-593|def _clamp_ev_to_supported(ev_candidate, ev_min=AUTO_EV_M...|
|`_quantize_ev_to_supported`|fn|priv|594-614|def _quantize_ev_to_supported(ev_value)|
|`_coerce_positive_luminance`|fn|priv|615-634|def _coerce_positive_luminance(value, fallback)|
|`_optimize_adaptive_ev_delta`|fn|priv|635-656|def _optimize_adaptive_ev_delta(auto_ev_inputs)|
|`_compute_auto_ev_value`|fn|priv|657-727|def _compute_auto_ev_value(raw_handle)|
|`_percentile`|fn|priv|691-701|def _percentile(percentile_value)|
|`_resolve_ev_value`|fn|priv|728-747|def _resolve_ev_value(raw_handle, ev_value, auto_ev_enabled)|
|`_parse_luminance_text_option`|fn|priv|748-768|def _parse_luminance_text_option(option_name, option_raw)|
|`_parse_gamma_option`|fn|priv|769-805|def _parse_gamma_option(gamma_raw)|
|`_parse_positive_float_option`|fn|priv|806-829|def _parse_positive_float_option(option_name, option_raw)|
|`_parse_tmo_passthrough_value`|fn|priv|830-846|def _parse_tmo_passthrough_value(option_name, option_raw)|
|`_parse_jpg_compression_option`|fn|priv|847-869|def _parse_jpg_compression_option(compression_raw)|
|`_parse_float_in_range_option`|fn|priv|870-895|def _parse_float_in_range_option(option_name, option_raw,...|
|`_parse_auto_adjust_options`|fn|priv|896-1002|def _parse_auto_adjust_options(auto_adjust_raw_values)|
|`_parse_auto_adjust_mode_option`|fn|priv|1003-1026|def _parse_auto_adjust_mode_option(auto_adjust_raw)|
|`_resolve_default_postprocess`|fn|priv|1027-1061|def _resolve_default_postprocess(enable_luminance, lumina...|
|`_parse_run_options`|fn|priv|1062-1261|def _parse_run_options(args)|
|`_load_image_dependencies`|fn|priv|1538-1576|def _load_image_dependencies()|
|`_parse_exif_datetime_to_timestamp`|fn|priv|1577-1607|def _parse_exif_datetime_to_timestamp(datetime_raw)|
|`_extract_dng_exif_payload_and_timestamp`|fn|priv|1608-1665|def _extract_dng_exif_payload_and_timestamp(pil_image_mod...|
|`_resolve_thumbnail_transpose_map`|fn|priv|1666-1697|def _resolve_thumbnail_transpose_map(pil_image_module)|
|`_apply_orientation_transform`|fn|priv|1698-1720|def _apply_orientation_transform(pil_image_module, pil_im...|
|`_build_oriented_thumbnail_jpeg_bytes`|fn|priv|1721-1722|def _build_oriented_thumbnail_jpeg_bytes(|
|`_coerce_exif_int_like_value`|fn|priv|1751-1793|def _coerce_exif_int_like_value(raw_value)|
|`_normalize_ifd_integer_like_values_for_piexif_dump`|fn|priv|1794-1927|def _normalize_ifd_integer_like_values_for_piexif_dump(pi...|
|`_refresh_output_jpg_exif_thumbnail_after_save`|fn|priv|1928-1933|def _refresh_output_jpg_exif_thumbnail_after_save(|
|`_set_output_file_timestamps`|fn|priv|1982-1996|def _set_output_file_timestamps(output_jpg, exif_timestamp)|
|`_sync_output_file_timestamps_from_exif`|fn|priv|1997-2013|def _sync_output_file_timestamps_from_exif(output_jpg, ex...|
|`_build_exposure_multipliers`|fn|priv|2014-2026|def _build_exposure_multipliers(ev_value)|
|`_write_bracket_images`|fn|priv|2027-2028|def _write_bracket_images(|
|`_order_bracket_paths`|fn|priv|2065-2090|def _order_bracket_paths(bracket_paths)|
|`_run_enfuse`|fn|priv|2091-2111|def _run_enfuse(bracket_paths, merged_tiff)|
|`_run_luminance_hdr_cli`|fn|priv|2112-2151|def _run_luminance_hdr_cli(bracket_paths, output_hdr_tiff...|
|`_convert_compression_to_quality`|fn|priv|2152-2164|def _convert_compression_to_quality(jpg_compression)|
|`_resolve_imagemagick_command`|fn|priv|2165-2182|def _resolve_imagemagick_command()|
|`_resolve_auto_adjust_opencv_dependencies`|fn|priv|2183-2206|def _resolve_auto_adjust_opencv_dependencies()|
|`_apply_validated_auto_adjust_pipeline`|fn|priv|2207-2208|def _apply_validated_auto_adjust_pipeline(|
|`_clamp01`|fn|priv|2289-2302|def _clamp01(np_module, values)|
|`_gaussian_kernel_2d`|fn|priv|2303-2325|def _gaussian_kernel_2d(np_module, sigma, radius=None)|
|`_rgb_to_hsl`|fn|priv|2326-2359|def _rgb_to_hsl(np_module, rgb)|
|`_hue_to_rgb`|fn|priv|2360-2390|def _hue_to_rgb(np_module, p_values, q_values, t_values)|
|`_hsl_to_rgb`|fn|priv|2391-2431|def _hsl_to_rgb(np_module, hue, saturation, lightness)|
|`_selective_blur_contrast_gated_vectorized`|fn|priv|2432-2433|def _selective_blur_contrast_gated_vectorized(|
|`_level_per_channel_adaptive`|fn|priv|2482-2504|def _level_per_channel_adaptive(np_module, rgb, low_pct=0...|
|`_sigmoidal_contrast`|fn|priv|2505-2529|def _sigmoidal_contrast(np_module, rgb, contrast=3.0, mid...|
|`logistic`|fn|pub|2520-2522|def logistic(z_values)|
|`_vibrance_hsl_gamma`|fn|priv|2530-2547|def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=...|
|`_gaussian_blur_rgb`|fn|priv|2548-2571|def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)|
|`_high_pass_math_gray`|fn|priv|2572-2595|def _high_pass_math_gray(cv2_module, np_module, rgb, blur...|
|`_overlay_composite`|fn|priv|2596-2617|def _overlay_composite(np_module, base_rgb, overlay_gray)|
|`_apply_validated_auto_adjust_pipeline_opencv`|fn|priv|2618-2619|def _apply_validated_auto_adjust_pipeline_opencv(|
|`_load_piexif_dependency`|fn|priv|2698-2715|def _load_piexif_dependency()|
|`_encode_jpg`|fn|priv|2716-2727|def _encode_jpg(|
|`_collect_processing_errors`|fn|priv|2883-2911|def _collect_processing_errors(rawpy_module)|
|`_is_supported_runtime_os`|fn|priv|2912-2931|def _is_supported_runtime_os()|
|`run`|fn|pub|2932-3096|def run(args)|
