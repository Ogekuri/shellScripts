...
# dng2hdr2jpg.py | Python | 3608L | 116 symbols | 19 imports | 76 comments
...
- var `DEFAULT_LUMINANCE_TMO = "reinhard02"` (L55)
...
- var `SUPPORTED_EV_VALUES = (0.5, 1.0, 1.5, 2.0)` (L59)
- var `AUTO_EV_LOW_PERCENTILE = 0.1` (L60)
- var `AUTO_EV_HIGH_PERCENTILE = 99.9` (L61)
- var `AUTO_EV_MEDIAN_PERCENTILE = 50.0` (L62)
- var `AUTO_EV_TARGET_SHADOW = 0.05` (L63)
- var `AUTO_EV_TARGET_HIGHLIGHT = 0.90` (L64)
- var `AUTO_EV_MEDIAN_TARGET = 0.5` (L65)
- var `AUTO_EV_MIN = min(SUPPORTED_EV_VALUES)` (L66)
- var `AUTO_EV_MAX = max(SUPPORTED_EV_VALUES)` (L67)
### class `class AutoAdjustOptions` `@dataclass(frozen=True)` (L245-272)
...
### class `class AutoBrightnessOptions` `@dataclass(frozen=True)` (L274-296)
...
### class `class PostprocessOptions` `@dataclass(frozen=True)` (L298-328)
...
### class `class LuminanceOptions` `@dataclass(frozen=True)` (L330-350)
...
### class `class AutoEvInputs` `@dataclass(frozen=True)` (L352-379)
...
### fn `def _print_box_table(headers, rows, header_rows=())` `priv` (L380-416)
...
### fn `def _border(left, middle, right)` `priv` (L400-402)
- @brief Print one Unicode box-drawing table.
...
### fn `def _line(values)` `priv` (L403-406)

### fn `def _build_two_line_operator_rows(operator_entries)` `priv` (L417-433)
...
### fn `def print_help(version)` (L434-607)
- @brief Print help text for the `dng2hdr2jpg` command.
- @details Documents required positional arguments, required mutually exclusive exposure selectors (`--ev` or `--auto-ev`), optional RAW gamma controls, shared postprocessing controls, backend selection, and luminance-hdr-cli tone-mapping options.
...
### fn `def _parse_ev_option(ev_raw)` `priv` (L608-632)
...
### fn `def _parse_auto_ev_option(auto_ev_raw)` `priv` (L633-650)
...
### fn `def _parse_auto_brightness_option(auto_brightness_raw)` `priv` (L651-668)
...
### fn `def _clamp_ev_to_supported(ev_candidate, ev_min=AUTO_EV_MIN, ev_max=AUTO_EV_MAX)` `priv` (L669-683)
...
### fn `def _quantize_ev_to_supported(ev_value)` `priv` (L684-704)
- @brief Quantize one EV value to nearest supported selector value.
- @details Chooses nearest value from `SUPPORTED_EV_VALUES` to preserve deterministic three-bracket behavior in downstream static multiplier and HDR command construction paths.
- @param ev_value {float} Clamped EV value.
- @return {float} Nearest supported EV selector value.
- @satisfies REQ-080, REQ-081

### fn `def _coerce_positive_luminance(value, fallback)` `priv` (L705-724)
...
### fn `def _optimize_adaptive_ev_delta(auto_ev_inputs)` `priv` (L725-746)
- @brief Compute adaptive EV delta from preview luminance statistics.
- @details Computes `ev_shadow=log2(target_shadow/p_low)` and `ev_high=log2(p_high/target_highlight)`, applies median-centering tie-break candidate `ev_median=abs(log2(median_target/p_median))`, clamps by `[ev_min, ev_max]`, and quantizes to nearest supported EV value.
- @param auto_ev_inputs {AutoEvInputs} Adaptive EV scalar inputs.
- @return {float} Quantized adaptive EV delta.
- @satisfies REQ-080, REQ-081

### fn `def _compute_auto_ev_value(raw_handle)` `priv` (L747-817)
- @brief Compute adaptive EV selector from RAW linear preview histogram.
- @details Generates one linear RAW preview using camera white balance, `no_auto_bright=True`, `gamma=(1.0, 1.0)`, and `user_flip=0`; computes luminance map and percentiles (`0.1`, `50.0`, `99.9`); then derives adaptive EV delta through constrained logarithmic optimization and quantization.
...
### fn `def _percentile(percentile_value)` `priv` (L781-791)
- @brief Compute adaptive EV selector from RAW linear preview histogram.
...
### fn `def _resolve_ev_value(raw_handle, ev_value, auto_ev_enabled)` `priv` (L818-837)
- @brief Resolve effective EV selector for static or adaptive mode.
- @details Returns explicit static `--ev` value when adaptive mode is not enabled. In adaptive mode, computes EV from RAW linear preview statistics.
- @param raw_handle {Any} Opened RAW handle from `rawpy.imread`.
...
### fn `def _parse_luminance_text_option(option_name, option_raw)` `priv` (L838-858)
...
### fn `def _parse_gamma_option(gamma_raw)` `priv` (L859-895)
...
### fn `def _parse_positive_float_option(option_name, option_raw)` `priv` (L896-919)
...
### fn `def _parse_tmo_passthrough_value(option_name, option_raw)` `priv` (L920-936)
...
### fn `def _parse_jpg_compression_option(compression_raw)` `priv` (L937-959)
...
### fn `def _parse_positive_int_option(option_name, option_raw)` `priv` (L960-982)
...
### fn `def _parse_auto_brightness_tile_grid_size_option(option_raw)` `priv` (L983-1006)
...
### fn `def _parse_float_exclusive_range_option(option_name, option_raw, min_value, max_value)` `priv` (L1007-1031)
...
### fn `def _parse_non_negative_float_option(option_name, option_raw)` `priv` (L1032-1054)
...
### fn `def _parse_float_in_range_option(option_name, option_raw, min_value, max_value)` `priv` (L1055-1080)
...
### fn `def _parse_auto_brightness_options(auto_brightness_raw_values)` `priv` (L1081-1151)
...
### fn `def _parse_auto_adjust_options(auto_adjust_raw_values)` `priv` (L1152-1258)
...
### fn `def _parse_auto_adjust_mode_option(auto_adjust_raw)` `priv` (L1259-1282)
...
### fn `def _resolve_default_postprocess(enable_luminance, luminance_tmo)` `priv` (L1283-1317)
...
### fn `def _load_image_dependencies()` `priv` (L1852-1890)
...
### fn `def _parse_exif_datetime_to_timestamp(datetime_raw)` `priv` (L1891-1921)
...
### fn `def _extract_dng_exif_payload_and_timestamp(pil_image_module, input_dng)` `priv` (L1922-1979)
...
### fn `def _resolve_thumbnail_transpose_map(pil_image_module)` `priv` (L1980-2011)
...
### fn `def _apply_orientation_transform(pil_image_module, pil_image, source_orientation)` `priv` (L2012-2034)
...
### fn `def _build_oriented_thumbnail_jpeg_bytes(` `priv` (L2035-2036)

### fn `def _coerce_exif_int_like_value(raw_value)` `priv` (L2065-2107)
...
### fn `def _normalize_ifd_integer_like_values_for_piexif_dump(piexif_module, exif_dict)` `priv` (L2108-2241)
...
### fn `def _refresh_output_jpg_exif_thumbnail_after_save(` `priv` (L2242-2247)

### fn `def _set_output_file_timestamps(output_jpg, exif_timestamp)` `priv` (L2296-2310)
...
### fn `def _sync_output_file_timestamps_from_exif(output_jpg, exif_timestamp)` `priv` (L2311-2327)
...
### fn `def _build_exposure_multipliers(ev_value)` `priv` (L2328-2340)
...
### fn `def _write_bracket_images(` `priv` (L2341-2342)

### fn `def _order_bracket_paths(bracket_paths)` `priv` (L2379-2404)
...
### fn `def _run_enfuse(bracket_paths, merged_tiff)` `priv` (L2405-2425)
...
### fn `def _run_luminance_hdr_cli(bracket_paths, output_hdr_tiff, ev_value, luminance_options)` `priv` (L2426-2465)
...
### fn `def _convert_compression_to_quality(jpg_compression)` `priv` (L2466-2478)
...
### fn `def _resolve_imagemagick_command()` `priv` (L2479-2496)
...
### fn `def _resolve_auto_adjust_opencv_dependencies()` `priv` (L2497-2521)
...
### fn `def _to_uint8_image_array(np_module, image_data)` `priv` (L2522-2566)
...
### fn `def _clip_histogram_autolevel_channel(` `priv` (L2567-2568)

### fn `def _apply_mean_gamma_correction_channel(` `priv` (L2600-2601)
...
### fn `def _apply_auto_brightness_rgb_uint8(` `priv` (L2628-2629)
...
### fn `def _apply_validated_auto_adjust_pipeline(` `priv` (L2693-2694)
...
### fn `def _clamp01(np_module, values)` `priv` (L2775-2788)
...
### fn `def _gaussian_kernel_2d(np_module, sigma, radius=None)` `priv` (L2789-2811)
...
### fn `def _rgb_to_hsl(np_module, rgb)` `priv` (L2812-2845)
...
### fn `def _hue_to_rgb(np_module, p_values, q_values, t_values)` `priv` (L2846-2876)
...
### fn `def _hsl_to_rgb(np_module, hue, saturation, lightness)` `priv` (L2877-2917)
...
### fn `def _selective_blur_contrast_gated_vectorized(` `priv` (L2918-2919)

### fn `def _level_per_channel_adaptive(np_module, rgb, low_pct=0.1, high_pct=99.9)` `priv` (L2968-2990)
...
### fn `def _sigmoidal_contrast(np_module, rgb, contrast=3.0, midpoint=0.5)` `priv` (L2991-3015)
...
### fn `def logistic(z_values)` (L3006-3008)
- @brief Execute sigmoidal contrast stage.
...
### fn `def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=0.8)` `priv` (L3016-3033)
...
### fn `def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)` `priv` (L3034-3057)
...
### fn `def _high_pass_math_gray(cv2_module, np_module, rgb, blur_sigma=2.5)` `priv` (L3058-3081)
...
### fn `def _overlay_composite(np_module, base_rgb, overlay_gray)` `priv` (L3082-3103)
...
### fn `def _apply_validated_auto_adjust_pipeline_opencv(` `priv` (L3104-3105)

### fn `def _load_piexif_dependency()` `priv` (L3184-3201)
...
### fn `def _encode_jpg(` `priv` (L3202-3213)

### fn `def _collect_processing_errors(rawpy_module)` `priv` (L3391-3419)
...
### fn `def _is_supported_runtime_os()` `priv` (L3420-3439)
...
### fn `def run(args)` (L3440-3608)
- @brief Execute `dng2hdr2jpg` command pipeline.
- @details Parses command options, validates dependencies, resolves static or adaptive EV selector, extracts three RAW brackets, executes selected `enfuse` flow or selected luminance-hdr-cli flow, writes JPG output, and guarantees temporary artifact cleanup through isolated temporary directory lifecycle.
...
|`SUPPORTED_EV_VALUES`|var|pub|59||
|`AUTO_EV_LOW_PERCENTILE`|var|pub|60||
|`AUTO_EV_HIGH_PERCENTILE`|var|pub|61||
|`AUTO_EV_MEDIAN_PERCENTILE`|var|pub|62||
|`AUTO_EV_TARGET_SHADOW`|var|pub|63||
|`AUTO_EV_TARGET_HIGHLIGHT`|var|pub|64||
|`AUTO_EV_MEDIAN_TARGET`|var|pub|65||
|`AUTO_EV_MIN`|var|pub|66||
|`AUTO_EV_MAX`|var|pub|67||
|`AutoAdjustOptions`|class|pub|245-272|class AutoAdjustOptions|
|`AutoBrightnessOptions`|class|pub|274-296|class AutoBrightnessOptions|
|`PostprocessOptions`|class|pub|298-328|class PostprocessOptions|
|`LuminanceOptions`|class|pub|330-350|class LuminanceOptions|
|`AutoEvInputs`|class|pub|352-379|class AutoEvInputs|
|`_print_box_table`|fn|priv|380-416|def _print_box_table(headers, rows, header_rows=())|
|`_border`|fn|priv|400-402|def _border(left, middle, right)|
|`_line`|fn|priv|403-406|def _line(values)|
|`_build_two_line_operator_rows`|fn|priv|417-433|def _build_two_line_operator_rows(operator_entries)|
|`print_help`|fn|pub|434-607|def print_help(version)|
|`_parse_ev_option`|fn|priv|608-632|def _parse_ev_option(ev_raw)|
|`_parse_auto_ev_option`|fn|priv|633-650|def _parse_auto_ev_option(auto_ev_raw)|
|`_parse_auto_brightness_option`|fn|priv|651-668|def _parse_auto_brightness_option(auto_brightness_raw)|
|`_clamp_ev_to_supported`|fn|priv|669-683|def _clamp_ev_to_supported(ev_candidate, ev_min=AUTO_EV_M...|
|`_quantize_ev_to_supported`|fn|priv|684-704|def _quantize_ev_to_supported(ev_value)|
|`_coerce_positive_luminance`|fn|priv|705-724|def _coerce_positive_luminance(value, fallback)|
|`_optimize_adaptive_ev_delta`|fn|priv|725-746|def _optimize_adaptive_ev_delta(auto_ev_inputs)|
|`_compute_auto_ev_value`|fn|priv|747-817|def _compute_auto_ev_value(raw_handle)|
|`_percentile`|fn|priv|781-791|def _percentile(percentile_value)|
|`_resolve_ev_value`|fn|priv|818-837|def _resolve_ev_value(raw_handle, ev_value, auto_ev_enabled)|
|`_parse_luminance_text_option`|fn|priv|838-858|def _parse_luminance_text_option(option_name, option_raw)|
|`_parse_gamma_option`|fn|priv|859-895|def _parse_gamma_option(gamma_raw)|
|`_parse_positive_float_option`|fn|priv|896-919|def _parse_positive_float_option(option_name, option_raw)|
|`_parse_tmo_passthrough_value`|fn|priv|920-936|def _parse_tmo_passthrough_value(option_name, option_raw)|
|`_parse_jpg_compression_option`|fn|priv|937-959|def _parse_jpg_compression_option(compression_raw)|
|`_parse_positive_int_option`|fn|priv|960-982|def _parse_positive_int_option(option_name, option_raw)|
|`_parse_auto_brightness_tile_grid_size_option`|fn|priv|983-1006|def _parse_auto_brightness_tile_grid_size_option(option_raw)|
|`_parse_float_exclusive_range_option`|fn|priv|1007-1031|def _parse_float_exclusive_range_option(option_name, opti...|
|`_parse_non_negative_float_option`|fn|priv|1032-1054|def _parse_non_negative_float_option(option_name, option_...|
|`_parse_float_in_range_option`|fn|priv|1055-1080|def _parse_float_in_range_option(option_name, option_raw,...|
|`_parse_auto_brightness_options`|fn|priv|1081-1151|def _parse_auto_brightness_options(auto_brightness_raw_va...|
|`_parse_auto_adjust_options`|fn|priv|1152-1258|def _parse_auto_adjust_options(auto_adjust_raw_values)|
|`_parse_auto_adjust_mode_option`|fn|priv|1259-1282|def _parse_auto_adjust_mode_option(auto_adjust_raw)|
|`_resolve_default_postprocess`|fn|priv|1283-1317|def _resolve_default_postprocess(enable_luminance, lumina...|
|`_parse_run_options`|fn|priv|1318-1517|def _parse_run_options(args)|
|`_load_image_dependencies`|fn|priv|1852-1890|def _load_image_dependencies()|
|`_parse_exif_datetime_to_timestamp`|fn|priv|1891-1921|def _parse_exif_datetime_to_timestamp(datetime_raw)|
|`_extract_dng_exif_payload_and_timestamp`|fn|priv|1922-1979|def _extract_dng_exif_payload_and_timestamp(pil_image_mod...|
|`_resolve_thumbnail_transpose_map`|fn|priv|1980-2011|def _resolve_thumbnail_transpose_map(pil_image_module)|
|`_apply_orientation_transform`|fn|priv|2012-2034|def _apply_orientation_transform(pil_image_module, pil_im...|
|`_build_oriented_thumbnail_jpeg_bytes`|fn|priv|2035-2036|def _build_oriented_thumbnail_jpeg_bytes(|
|`_coerce_exif_int_like_value`|fn|priv|2065-2107|def _coerce_exif_int_like_value(raw_value)|
|`_normalize_ifd_integer_like_values_for_piexif_dump`|fn|priv|2108-2241|def _normalize_ifd_integer_like_values_for_piexif_dump(pi...|
|`_refresh_output_jpg_exif_thumbnail_after_save`|fn|priv|2242-2247|def _refresh_output_jpg_exif_thumbnail_after_save(|
|`_set_output_file_timestamps`|fn|priv|2296-2310|def _set_output_file_timestamps(output_jpg, exif_timestamp)|
|`_sync_output_file_timestamps_from_exif`|fn|priv|2311-2327|def _sync_output_file_timestamps_from_exif(output_jpg, ex...|
|`_build_exposure_multipliers`|fn|priv|2328-2340|def _build_exposure_multipliers(ev_value)|
|`_write_bracket_images`|fn|priv|2341-2342|def _write_bracket_images(|
|`_order_bracket_paths`|fn|priv|2379-2404|def _order_bracket_paths(bracket_paths)|
|`_run_enfuse`|fn|priv|2405-2425|def _run_enfuse(bracket_paths, merged_tiff)|
|`_run_luminance_hdr_cli`|fn|priv|2426-2465|def _run_luminance_hdr_cli(bracket_paths, output_hdr_tiff...|
|`_convert_compression_to_quality`|fn|priv|2466-2478|def _convert_compression_to_quality(jpg_compression)|
|`_resolve_imagemagick_command`|fn|priv|2479-2496|def _resolve_imagemagick_command()|
|`_resolve_auto_adjust_opencv_dependencies`|fn|priv|2497-2521|def _resolve_auto_adjust_opencv_dependencies()|
|`_to_uint8_image_array`|fn|priv|2522-2566|def _to_uint8_image_array(np_module, image_data)|
|`_clip_histogram_autolevel_channel`|fn|priv|2567-2568|def _clip_histogram_autolevel_channel(|
|`_apply_mean_gamma_correction_channel`|fn|priv|2600-2601|def _apply_mean_gamma_correction_channel(|
|`_apply_auto_brightness_rgb_uint8`|fn|priv|2628-2629|def _apply_auto_brightness_rgb_uint8(|
|`_apply_validated_auto_adjust_pipeline`|fn|priv|2693-2694|def _apply_validated_auto_adjust_pipeline(|
|`_clamp01`|fn|priv|2775-2788|def _clamp01(np_module, values)|
|`_gaussian_kernel_2d`|fn|priv|2789-2811|def _gaussian_kernel_2d(np_module, sigma, radius=None)|
|`_rgb_to_hsl`|fn|priv|2812-2845|def _rgb_to_hsl(np_module, rgb)|
|`_hue_to_rgb`|fn|priv|2846-2876|def _hue_to_rgb(np_module, p_values, q_values, t_values)|
|`_hsl_to_rgb`|fn|priv|2877-2917|def _hsl_to_rgb(np_module, hue, saturation, lightness)|
|`_selective_blur_contrast_gated_vectorized`|fn|priv|2918-2919|def _selective_blur_contrast_gated_vectorized(|
|`_level_per_channel_adaptive`|fn|priv|2968-2990|def _level_per_channel_adaptive(np_module, rgb, low_pct=0...|
|`_sigmoidal_contrast`|fn|priv|2991-3015|def _sigmoidal_contrast(np_module, rgb, contrast=3.0, mid...|
|`logistic`|fn|pub|3006-3008|def logistic(z_values)|
|`_vibrance_hsl_gamma`|fn|priv|3016-3033|def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=...|
|`_gaussian_blur_rgb`|fn|priv|3034-3057|def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)|
|`_high_pass_math_gray`|fn|priv|3058-3081|def _high_pass_math_gray(cv2_module, np_module, rgb, blur...|
|`_overlay_composite`|fn|priv|3082-3103|def _overlay_composite(np_module, base_rgb, overlay_gray)|
|`_apply_validated_auto_adjust_pipeline_opencv`|fn|priv|3104-3105|def _apply_validated_auto_adjust_pipeline_opencv(|
|`_load_piexif_dependency`|fn|priv|3184-3201|def _load_piexif_dependency()|
|`_encode_jpg`|fn|priv|3202-3213|def _encode_jpg(|
|`_collect_processing_errors`|fn|priv|3391-3419|def _collect_processing_errors(rawpy_module)|
|`_is_supported_runtime_os`|fn|priv|3420-3439|def _is_supported_runtime_os()|
|`run`|fn|pub|3440-3608|def run(args)|
