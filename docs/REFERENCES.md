...
# dng2hdr2jpg.py | Python | 4249L | 141 symbols | 19 imports | 93 comments
...
- var `DEFAULT_AB_TARGET_GREY = 0.18` (L48)
- var `DEFAULT_AB_MAX_GAIN = 4.0` (L49)
- var `DEFAULT_AB_MIN_GAIN = 0.95` (L50)
- var `DEFAULT_AB_P98_HIGHLIGHT_MAX = 0.90` (L51)
- var `DEFAULT_AB_CORRECTION_STRENGTH = 0.35` (L52)
- var `DEFAULT_AB_MAX_EV_CORRECTION = 0.5` (L53)
...
### class `class AutoAdjustOptions` `@dataclass(frozen=True)` (L254-281)
...
### class `class AutoBrightnessOptions` `@dataclass(frozen=True)` (L283-300)
...
### class `class PostprocessOptions` `@dataclass(frozen=True)` (L302-332)
...
### class `class LuminanceOptions` `@dataclass(frozen=True)` (L334-354)
...
### class `class AutoEvInputs` `@dataclass(frozen=True)` (L356-383)
...
### fn `def _print_box_table(headers, rows, header_rows=())` `priv` (L384-420)
...
### fn `def _border(left, middle, right)` `priv` (L404-406)
- @brief Print one Unicode box-drawing table.
...
### fn `def _line(values)` `priv` (L407-410)

### fn `def _build_two_line_operator_rows(operator_entries)` `priv` (L421-437)
...
### fn `def print_help(version)` (L438-621)
...
### fn `def _calculate_max_ev_from_bits(bits_per_color)` `priv` (L622-640)
...
### fn `def _calculate_safe_ev_zero_max(base_max_ev)` `priv` (L641-653)
...
### fn `def _derive_supported_ev_zero_values(base_max_ev)` `priv` (L654-670)
...
### fn `def _derive_supported_ev_values(bits_per_color, ev_zero=0.0)` `priv` (L671-699)
...
### fn `def _detect_dng_bits_per_color(raw_handle)` `priv` (L700-745)
...
### fn `def _is_ev_value_on_supported_step(ev_value)` `priv` (L746-759)
...
### fn `def _parse_ev_option(ev_raw)` `priv` (L760-791)
...
### fn `def _parse_ev_zero_option(ev_zero_raw)` `priv` (L792-822)
...
### fn `def _parse_auto_ev_option(auto_ev_raw)` `priv` (L823-840)
...
### fn `def _parse_auto_zero_option(auto_zero_raw)` `priv` (L841-858)
...
### fn `def _parse_percentage_option(option_name, option_raw)` `priv` (L859-881)
...
### fn `def _parse_auto_brightness_option(auto_brightness_raw)` `priv` (L882-899)
...
### fn `def _clamp_ev_to_supported(ev_candidate, ev_values)` `priv` (L900-913)
...
### fn `def _quantize_ev_to_supported(ev_value, ev_values)` `priv` (L914-935)
...
### fn `def _quantize_ev_toward_zero_step(ev_value, step=EV_STEP)` `priv` (L936-957)
...
### fn `def _apply_auto_percentage_scaling(ev_value, percentage)` `priv` (L958-972)
...
### fn `def _extract_normalized_preview_luminance_stats(raw_handle)` `priv` (L973-1032)
...
### fn `def _percentile(percentile_value)` `priv` (L1007-1017)
- @brief Extract normalized preview luminance percentiles from RAW handle.
...
### fn `def _coerce_positive_luminance(value, fallback)` `priv` (L1033-1052)
...
### fn `def _derive_scene_key_preserving_median_target(p_median)` `priv` (L1053-1071)
...
### fn `def _optimize_auto_zero(auto_ev_inputs)` `priv` (L1072-1095)
...
### fn `def _optimize_adaptive_ev_delta(auto_ev_inputs)` `priv` (L1096-1125)
...
### fn `def _compute_auto_ev_value_from_stats(` `priv` (L1126-1131)

### fn `def _compute_auto_ev_value(raw_handle, supported_ev_values=None, ev_zero=0.0)` `priv` (L1159-1186)
...
### fn `def _resolve_ev_zero(` `priv` (L1187-1194)

### fn `def _resolve_ev_value(` `priv` (L1245-1252)
...
### fn `def _parse_luminance_text_option(option_name, option_raw)` `priv` (L1305-1325)
...
### fn `def _parse_gamma_option(gamma_raw)` `priv` (L1326-1362)
...
### fn `def _parse_positive_float_option(option_name, option_raw)` `priv` (L1363-1386)
...
### fn `def _parse_tmo_passthrough_value(option_name, option_raw)` `priv` (L1387-1403)
...
### fn `def _parse_jpg_compression_option(compression_raw)` `priv` (L1404-1426)
...
### fn `def _parse_float_exclusive_range_option(option_name, option_raw, min_value, max_value)` `priv` (L1427-1451)
...
### fn `def _parse_non_negative_float_option(option_name, option_raw)` `priv` (L1452-1474)
...
### fn `def _parse_float_in_range_option(option_name, option_raw, min_value, max_value)` `priv` (L1475-1500)
...
### fn `def _parse_auto_brightness_options(auto_brightness_raw_values)` `priv` (L1501-1524)
...
### fn `def _parse_auto_adjust_options(auto_adjust_raw_values)` `priv` (L1525-1631)
...
### fn `def _parse_auto_adjust_mode_option(auto_adjust_raw)` `priv` (L1632-1655)
...
### fn `def _resolve_default_postprocess(enable_luminance, luminance_tmo)` `priv` (L1656-1698)
...
### fn `def _load_image_dependencies()` `priv` (L2334-2372)
...
### fn `def _parse_exif_datetime_to_timestamp(datetime_raw)` `priv` (L2373-2403)
...
### fn `def _extract_dng_exif_payload_and_timestamp(pil_image_module, input_dng)` `priv` (L2404-2461)
...
### fn `def _resolve_thumbnail_transpose_map(pil_image_module)` `priv` (L2462-2493)
...
### fn `def _apply_orientation_transform(pil_image_module, pil_image, source_orientation)` `priv` (L2494-2516)
...
### fn `def _build_oriented_thumbnail_jpeg_bytes(` `priv` (L2517-2518)

### fn `def _coerce_exif_int_like_value(raw_value)` `priv` (L2547-2589)
...
### fn `def _normalize_ifd_integer_like_values_for_piexif_dump(piexif_module, exif_dict)` `priv` (L2590-2723)
...
### fn `def _refresh_output_jpg_exif_thumbnail_after_save(` `priv` (L2724-2729)

### fn `def _set_output_file_timestamps(output_jpg, exif_timestamp)` `priv` (L2778-2792)
...
### fn `def _sync_output_file_timestamps_from_exif(output_jpg, exif_timestamp)` `priv` (L2793-2809)
...
### fn `def _build_exposure_multipliers(ev_value, ev_zero=0.0)` `priv` (L2810-2828)
...
### fn `def _write_bracket_images(` `priv` (L2829-2830)

### fn `def _order_bracket_paths(bracket_paths)` `priv` (L2867-2892)
...
### fn `def _run_enfuse(bracket_paths, merged_tiff)` `priv` (L2893-2913)
...
### fn `def _run_luminance_hdr_cli(` `priv` (L2914-2915)

### fn `def _convert_compression_to_quality(jpg_compression)` `priv` (L2967-2979)
...
### fn `def _resolve_imagemagick_command()` `priv` (L2980-2997)
...
### fn `def _resolve_auto_adjust_opencv_dependencies()` `priv` (L2998-3022)
...
### fn `def _to_uint8_image_array(np_module, image_data)` `priv` (L3023-3069)
...
### fn `def _to_uint16_image_array(np_module, image_data)` `priv` (L3070-3114)
...
### fn `def _to_linear_srgb(np_module, image_srgb)` `priv` (L3115-3132)
...
### fn `def _from_linear_srgb(np_module, image_linear)` `priv` (L3133-3150)
...
### fn `def _compute_bt709_luminance(np_module, linear_rgb)` `priv` (L3151-3168)
...
### fn `def _apply_highlight_rolloff(np_module, linear_rgb)` `priv` (L3169-3187)
...
### fn `def _apply_auto_brightness_rgb_uint8(` `priv` (L3188-3189)

### fn `def _apply_validated_auto_adjust_pipeline(` `priv` (L3248-3249)
...
### fn `def _clamp01(np_module, values)` `priv` (L3330-3343)
...
### fn `def _gaussian_kernel_2d(np_module, sigma, radius=None)` `priv` (L3344-3366)
...
### fn `def _rgb_to_hsl(np_module, rgb)` `priv` (L3367-3400)
...
### fn `def _hue_to_rgb(np_module, p_values, q_values, t_values)` `priv` (L3401-3431)
...
### fn `def _hsl_to_rgb(np_module, hue, saturation, lightness)` `priv` (L3432-3472)
...
### fn `def _selective_blur_contrast_gated_vectorized(` `priv` (L3473-3474)

### fn `def _level_per_channel_adaptive(np_module, rgb, low_pct=0.1, high_pct=99.9)` `priv` (L3523-3545)
...
### fn `def _sigmoidal_contrast(np_module, rgb, contrast=3.0, midpoint=0.5)` `priv` (L3546-3570)
...
### fn `def logistic(z_values)` (L3561-3563)
- @brief Execute sigmoidal contrast stage.
...
### fn `def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=0.8)` `priv` (L3571-3588)
...
### fn `def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)` `priv` (L3589-3612)
...
### fn `def _high_pass_math_gray(cv2_module, np_module, rgb, blur_sigma=2.5)` `priv` (L3613-3636)
...
### fn `def _overlay_composite(np_module, base_rgb, overlay_gray)` `priv` (L3637-3658)
...
### fn `def _apply_validated_auto_adjust_pipeline_opencv(` `priv` (L3659-3660)

### fn `def _load_piexif_dependency()` `priv` (L3739-3756)
...
### fn `def _encode_jpg(` `priv` (L3757-3769)

### fn `def _collect_processing_errors(rawpy_module)` `priv` (L3977-4005)
...
### fn `def _is_supported_runtime_os()` `priv` (L4006-4025)
...
|`DEFAULT_AB_TARGET_GREY`|var|pub|48||
|`DEFAULT_AB_MAX_GAIN`|var|pub|49||
|`DEFAULT_AB_MIN_GAIN`|var|pub|50||
|`DEFAULT_AB_P98_HIGHLIGHT_MAX`|var|pub|51||
|`DEFAULT_AB_CORRECTION_STRENGTH`|var|pub|52||
|`DEFAULT_AB_MAX_EV_CORRECTION`|var|pub|53||
...
|`AutoAdjustOptions`|class|pub|254-281|class AutoAdjustOptions|
|`AutoBrightnessOptions`|class|pub|283-300|class AutoBrightnessOptions|
|`PostprocessOptions`|class|pub|302-332|class PostprocessOptions|
|`LuminanceOptions`|class|pub|334-354|class LuminanceOptions|
|`AutoEvInputs`|class|pub|356-383|class AutoEvInputs|
|`_print_box_table`|fn|priv|384-420|def _print_box_table(headers, rows, header_rows=())|
|`_border`|fn|priv|404-406|def _border(left, middle, right)|
|`_line`|fn|priv|407-410|def _line(values)|
|`_build_two_line_operator_rows`|fn|priv|421-437|def _build_two_line_operator_rows(operator_entries)|
|`print_help`|fn|pub|438-621|def print_help(version)|
|`_calculate_max_ev_from_bits`|fn|priv|622-640|def _calculate_max_ev_from_bits(bits_per_color)|
|`_calculate_safe_ev_zero_max`|fn|priv|641-653|def _calculate_safe_ev_zero_max(base_max_ev)|
|`_derive_supported_ev_zero_values`|fn|priv|654-670|def _derive_supported_ev_zero_values(base_max_ev)|
|`_derive_supported_ev_values`|fn|priv|671-699|def _derive_supported_ev_values(bits_per_color, ev_zero=0.0)|
|`_detect_dng_bits_per_color`|fn|priv|700-745|def _detect_dng_bits_per_color(raw_handle)|
|`_is_ev_value_on_supported_step`|fn|priv|746-759|def _is_ev_value_on_supported_step(ev_value)|
|`_parse_ev_option`|fn|priv|760-791|def _parse_ev_option(ev_raw)|
|`_parse_ev_zero_option`|fn|priv|792-822|def _parse_ev_zero_option(ev_zero_raw)|
|`_parse_auto_ev_option`|fn|priv|823-840|def _parse_auto_ev_option(auto_ev_raw)|
|`_parse_auto_zero_option`|fn|priv|841-858|def _parse_auto_zero_option(auto_zero_raw)|
|`_parse_percentage_option`|fn|priv|859-881|def _parse_percentage_option(option_name, option_raw)|
|`_parse_auto_brightness_option`|fn|priv|882-899|def _parse_auto_brightness_option(auto_brightness_raw)|
|`_clamp_ev_to_supported`|fn|priv|900-913|def _clamp_ev_to_supported(ev_candidate, ev_values)|
|`_quantize_ev_to_supported`|fn|priv|914-935|def _quantize_ev_to_supported(ev_value, ev_values)|
|`_quantize_ev_toward_zero_step`|fn|priv|936-957|def _quantize_ev_toward_zero_step(ev_value, step=EV_STEP)|
|`_apply_auto_percentage_scaling`|fn|priv|958-972|def _apply_auto_percentage_scaling(ev_value, percentage)|
|`_extract_normalized_preview_luminance_stats`|fn|priv|973-1032|def _extract_normalized_preview_luminance_stats(raw_handle)|
|`_percentile`|fn|priv|1007-1017|def _percentile(percentile_value)|
|`_coerce_positive_luminance`|fn|priv|1033-1052|def _coerce_positive_luminance(value, fallback)|
|`_derive_scene_key_preserving_median_target`|fn|priv|1053-1071|def _derive_scene_key_preserving_median_target(p_median)|
|`_optimize_auto_zero`|fn|priv|1072-1095|def _optimize_auto_zero(auto_ev_inputs)|
|`_optimize_adaptive_ev_delta`|fn|priv|1096-1125|def _optimize_adaptive_ev_delta(auto_ev_inputs)|
|`_compute_auto_ev_value_from_stats`|fn|priv|1126-1131|def _compute_auto_ev_value_from_stats(|
|`_compute_auto_ev_value`|fn|priv|1159-1186|def _compute_auto_ev_value(raw_handle, supported_ev_value...|
|`_resolve_ev_zero`|fn|priv|1187-1194|def _resolve_ev_zero(|
|`_resolve_ev_value`|fn|priv|1245-1252|def _resolve_ev_value(|
|`_parse_luminance_text_option`|fn|priv|1305-1325|def _parse_luminance_text_option(option_name, option_raw)|
|`_parse_gamma_option`|fn|priv|1326-1362|def _parse_gamma_option(gamma_raw)|
|`_parse_positive_float_option`|fn|priv|1363-1386|def _parse_positive_float_option(option_name, option_raw)|
|`_parse_tmo_passthrough_value`|fn|priv|1387-1403|def _parse_tmo_passthrough_value(option_name, option_raw)|
|`_parse_jpg_compression_option`|fn|priv|1404-1426|def _parse_jpg_compression_option(compression_raw)|
|`_parse_float_exclusive_range_option`|fn|priv|1427-1451|def _parse_float_exclusive_range_option(option_name, opti...|
|`_parse_non_negative_float_option`|fn|priv|1452-1474|def _parse_non_negative_float_option(option_name, option_...|
|`_parse_float_in_range_option`|fn|priv|1475-1500|def _parse_float_in_range_option(option_name, option_raw,...|
|`_parse_auto_brightness_options`|fn|priv|1501-1524|def _parse_auto_brightness_options(auto_brightness_raw_va...|
|`_parse_auto_adjust_options`|fn|priv|1525-1631|def _parse_auto_adjust_options(auto_adjust_raw_values)|
|`_parse_auto_adjust_mode_option`|fn|priv|1632-1655|def _parse_auto_adjust_mode_option(auto_adjust_raw)|
|`_resolve_default_postprocess`|fn|priv|1656-1698|def _resolve_default_postprocess(enable_luminance, lumina...|
|`_parse_run_options`|fn|priv|1699-1898|def _parse_run_options(args)|
|`_load_image_dependencies`|fn|priv|2334-2372|def _load_image_dependencies()|
|`_parse_exif_datetime_to_timestamp`|fn|priv|2373-2403|def _parse_exif_datetime_to_timestamp(datetime_raw)|
|`_extract_dng_exif_payload_and_timestamp`|fn|priv|2404-2461|def _extract_dng_exif_payload_and_timestamp(pil_image_mod...|
|`_resolve_thumbnail_transpose_map`|fn|priv|2462-2493|def _resolve_thumbnail_transpose_map(pil_image_module)|
|`_apply_orientation_transform`|fn|priv|2494-2516|def _apply_orientation_transform(pil_image_module, pil_im...|
|`_build_oriented_thumbnail_jpeg_bytes`|fn|priv|2517-2518|def _build_oriented_thumbnail_jpeg_bytes(|
|`_coerce_exif_int_like_value`|fn|priv|2547-2589|def _coerce_exif_int_like_value(raw_value)|
|`_normalize_ifd_integer_like_values_for_piexif_dump`|fn|priv|2590-2723|def _normalize_ifd_integer_like_values_for_piexif_dump(pi...|
|`_refresh_output_jpg_exif_thumbnail_after_save`|fn|priv|2724-2729|def _refresh_output_jpg_exif_thumbnail_after_save(|
|`_set_output_file_timestamps`|fn|priv|2778-2792|def _set_output_file_timestamps(output_jpg, exif_timestamp)|
|`_sync_output_file_timestamps_from_exif`|fn|priv|2793-2809|def _sync_output_file_timestamps_from_exif(output_jpg, ex...|
|`_build_exposure_multipliers`|fn|priv|2810-2828|def _build_exposure_multipliers(ev_value, ev_zero=0.0)|
|`_write_bracket_images`|fn|priv|2829-2830|def _write_bracket_images(|
|`_order_bracket_paths`|fn|priv|2867-2892|def _order_bracket_paths(bracket_paths)|
|`_run_enfuse`|fn|priv|2893-2913|def _run_enfuse(bracket_paths, merged_tiff)|
|`_run_luminance_hdr_cli`|fn|priv|2914-2915|def _run_luminance_hdr_cli(|
|`_convert_compression_to_quality`|fn|priv|2967-2979|def _convert_compression_to_quality(jpg_compression)|
|`_resolve_imagemagick_command`|fn|priv|2980-2997|def _resolve_imagemagick_command()|
|`_resolve_auto_adjust_opencv_dependencies`|fn|priv|2998-3022|def _resolve_auto_adjust_opencv_dependencies()|
|`_to_uint8_image_array`|fn|priv|3023-3069|def _to_uint8_image_array(np_module, image_data)|
|`_to_uint16_image_array`|fn|priv|3070-3114|def _to_uint16_image_array(np_module, image_data)|
|`_to_linear_srgb`|fn|priv|3115-3132|def _to_linear_srgb(np_module, image_srgb)|
|`_from_linear_srgb`|fn|priv|3133-3150|def _from_linear_srgb(np_module, image_linear)|
|`_compute_bt709_luminance`|fn|priv|3151-3168|def _compute_bt709_luminance(np_module, linear_rgb)|
|`_apply_highlight_rolloff`|fn|priv|3169-3187|def _apply_highlight_rolloff(np_module, linear_rgb)|
|`_apply_auto_brightness_rgb_uint8`|fn|priv|3188-3189|def _apply_auto_brightness_rgb_uint8(|
|`_apply_validated_auto_adjust_pipeline`|fn|priv|3248-3249|def _apply_validated_auto_adjust_pipeline(|
|`_clamp01`|fn|priv|3330-3343|def _clamp01(np_module, values)|
|`_gaussian_kernel_2d`|fn|priv|3344-3366|def _gaussian_kernel_2d(np_module, sigma, radius=None)|
|`_rgb_to_hsl`|fn|priv|3367-3400|def _rgb_to_hsl(np_module, rgb)|
|`_hue_to_rgb`|fn|priv|3401-3431|def _hue_to_rgb(np_module, p_values, q_values, t_values)|
|`_hsl_to_rgb`|fn|priv|3432-3472|def _hsl_to_rgb(np_module, hue, saturation, lightness)|
|`_selective_blur_contrast_gated_vectorized`|fn|priv|3473-3474|def _selective_blur_contrast_gated_vectorized(|
|`_level_per_channel_adaptive`|fn|priv|3523-3545|def _level_per_channel_adaptive(np_module, rgb, low_pct=0...|
|`_sigmoidal_contrast`|fn|priv|3546-3570|def _sigmoidal_contrast(np_module, rgb, contrast=3.0, mid...|
|`logistic`|fn|pub|3561-3563|def logistic(z_values)|
|`_vibrance_hsl_gamma`|fn|priv|3571-3588|def _vibrance_hsl_gamma(np_module, rgb, saturation_gamma=...|
|`_gaussian_blur_rgb`|fn|priv|3589-3612|def _gaussian_blur_rgb(cv2_module, np_module, rgb, sigma)|
|`_high_pass_math_gray`|fn|priv|3613-3636|def _high_pass_math_gray(cv2_module, np_module, rgb, blur...|
|`_overlay_composite`|fn|priv|3637-3658|def _overlay_composite(np_module, base_rgb, overlay_gray)|
|`_apply_validated_auto_adjust_pipeline_opencv`|fn|priv|3659-3660|def _apply_validated_auto_adjust_pipeline_opencv(|
|`_load_piexif_dependency`|fn|priv|3739-3756|def _load_piexif_dependency()|
|`_encode_jpg`|fn|priv|3757-3769|def _encode_jpg(|
|`_collect_processing_errors`|fn|priv|3977-4005|def _collect_processing_errors(rawpy_module)|
|`_is_supported_runtime_os`|fn|priv|4006-4025|def _is_supported_runtime_os()|
|`run`|fn|pub|4026-4225|def run(args)|
