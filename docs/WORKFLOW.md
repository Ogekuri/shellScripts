...
- Internal Call-Trace Tree:
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`), then execute in-memory 16-bit postprocess, optional in-memory 16-bit `magic_retouch`, and final JPG encoding [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, EV selector, raw gamma selector, mandatory backend selector, postprocess factors, JPEG compression, optional `--magic-retouch` controls (`--magic-filter`, `--magic-kernel-size`, bilateral controls, sharpen strength), reject legacy magic options, and parse luminance backend options including default `flat` hdr-weight, default `reinhard02` tone mapping, and explicit `--tmo*` passthrough selectors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._parse_positive_int_option(...)`: Validate positive integer magic-retouch options [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._parse_magic_filter_option(...)`: Validate `--magic-filter` against supported OpenCV filters [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._parse_magic_kernel_size_option(...)`: Validate odd positive `--magic-kernel-size` value [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._magic_retouch(...)`: Execute optional deterministic OpenCV filter stage (`box`, `boxfilter`, `gaussian`, `median`, `bilateral`, `sharpen`) after postprocess and before JPG conversion, using neutral defaults [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._to_float01_from_u16(...)`: Normalize uint16 payload to float working domain [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._to_u16_from_float01(...)`: Convert normalized float payload back to uint16 [`src/shell_scripts/commands/dng2hdr2jpg.py`]
