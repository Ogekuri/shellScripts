...
- Internal Call-Trace Tree:
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    - `capture_terminal_state(...)`: Snapshot stdin termios attributes when stdin is a TTY and `termios` support is available [`src/shell_scripts/utils.py`]
    ...
    - `print_help(...)`: Print grouped global help sections or command-specific help [`src/shell_scripts/core.py`]
    ...
    - `do_upgrade(...)`: Linux-only self-upgrade wrapper [`src/shell_scripts/core.py`]
      ...
      - `require_shell_command_executables(...)`: Validate executable tokens used by configured shell command before subprocess execution [`src/shell_scripts/utils.py`]
        ...
        - `is_executable_command(...)`: Validate command-token or path executability on current runtime OS [`src/shell_scripts/utils.py`]
          - `resolve_executable_command(...)`: Resolve command token/path to runnable executable path [`src/shell_scripts/utils.py`]
            - `_resolve_executable_file(...)`: Resolve executable-file path with OS-specific extension semantics [`src/shell_scripts/utils.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      - `ai_install.run(...)`: Multi-tool AI CLI installer dispatcher with fixed selector parsing and unknown-option rejection for removed `--extra` behavior [`src/shell_scripts/commands/ai_install.py`]
        - `ai_install._install_npm_tool(...)`: Run OS-aware NPM global installation for selected tool, omitting `sudo` on Linux/Windows and applying `sudo` on other runtimes, with one retry for Windows Copilot transient install failures and boolean success result [`src/shell_scripts/commands/ai_install.py`]
          ...
          - `require_commands(...)`: Resolve and validate required executable tokens before subprocess execution [`src/shell_scripts/utils.py`]
            - `resolve_executable_command(...)`: Resolve command token/path to runnable executable path [`src/shell_scripts/utils.py`]
              - `_resolve_executable_file(...)`: Resolve executable-file path with OS-specific extension semantics [`src/shell_scripts/utils.py`]
        - `ai_install._install_pi(...)`: Install pi.dev CLI via npm helper and then install configured pi packages when npm installation succeeds [`src/shell_scripts/commands/ai_install.py`]
          - `ai_install._install_npm_tool(...)`: Install pi.dev CLI package with OS-aware npm strategy and return success state [`src/shell_scripts/commands/ai_install.py`]
          - `require_commands(...)`: Resolve and validate `pi` executable before post-install package subprocess execution [`src/shell_scripts/utils.py`]
            - `resolve_executable_command(...)`: Resolve command token/path to runnable executable path [`src/shell_scripts/utils.py`]
              - `_resolve_executable_file(...)`: Resolve executable-file path with OS-specific extension semantics [`src/shell_scripts/utils.py`]
        - `ai_install._install_claude(...)`: Download latest Claude metadata and resolve runtime-OS artifact candidate before installing binary under user home [`src/shell_scripts/commands/ai_install.py`]
          - `get_runtime_os(...)`: Read cached runtime OS token with lazy initialization [`src/shell_scripts/utils.py`]
            - `detect_runtime_os(...)`: Detect and cache normalized runtime OS token [`src/shell_scripts/utils.py`]
        - `ai_install._install_kiro(...)`: Gate unsupported Windows/macOS, resolve Linux package URL from official stable manifest, then download/extract/copy Kiro binaries into local bin [`src/shell_scripts/commands/ai_install.py`]
          - `get_runtime_os(...)`: Read cached runtime OS token with lazy initialization [`src/shell_scripts/utils.py`]
            - `detect_runtime_os(...)`: Detect and cache normalized runtime OS token [`src/shell_scripts/utils.py`]
          - `ai_install._normalize_kiro_linux_arch(...)`: Normalize Linux architecture token for manifest filtering [`src/shell_scripts/commands/ai_install.py`]
          - `ai_install._detect_kiro_linux_libc(...)`: Classify runtime Linux libc class for manifest filtering [`src/shell_scripts/commands/ai_install.py`]
          - `ai_install._resolve_kiro_linux_download_path(...)`: Select Linux headless ZIP path from manifest package list [`src/shell_scripts/commands/ai_install.py`]
      ...
      - `claude.run(...)`: Launch Claude CLI in project context [`src/shell_scripts/commands/claude.py`]
        - `require_project_root(...)`: Enforce git-root context or terminate process [`src/shell_scripts/utils.py`]
          - `get_project_root(...)`: Resolve git top-level directory by invoking git command [`src/shell_scripts/utils.py`]
        - `require_commands(...)`: Resolve and validate command executable path before subprocess invocation [`src/shell_scripts/utils.py`]
      - `codex.run(...)`: Launch Codex CLI with project-scoped `CODEX_HOME` and bidirectional auth file synchronization around `codex --yolo`, including informational copy evidence output [`src/shell_scripts/commands/codex.py`]
        - `require_project_root(...)`: Enforce git-root context or terminate process [`src/shell_scripts/utils.py`]
          - `get_project_root(...)`: Resolve git top-level directory by invoking git command [`src/shell_scripts/utils.py`]
      ...
      - `copilot.run(...)`: Launch Copilot CLI in project context via command token `copilot --yolo --allow-all-tools --no-auto-update` [`src/shell_scripts/commands/copilot.py`]
        - `require_project_root(...)`: Enforce git-root context or terminate process [`src/shell_scripts/utils.py`]
          - `get_project_root(...)`: Resolve git top-level directory by invoking git command [`src/shell_scripts/utils.py`]
        - `require_commands(...)`: Resolve and validate command executable path before subprocess invocation [`src/shell_scripts/utils.py`]
      - `gemini.run(...)`: Launch Gemini CLI in project context via command token `gemini --yolo` [`src/shell_scripts/commands/gemini.py`]
        - `require_project_root(...)`: Enforce git-root context or terminate process [`src/shell_scripts/utils.py`]
          - `get_project_root(...)`: Resolve git top-level directory by invoking git command [`src/shell_scripts/utils.py`]
        - `require_commands(...)`: Resolve and validate command executable path before subprocess invocation [`src/shell_scripts/utils.py`]
      - `kiro.run(...)`: Launch Kiro CLI in project context via command token `kiro-cli` [`src/shell_scripts/commands/kiro.py`]
        - `require_project_root(...)`: Enforce git-root context or terminate process [`src/shell_scripts/utils.py`]
          - `get_project_root(...)`: Resolve git top-level directory by invoking git command [`src/shell_scripts/utils.py`]
        - `require_commands(...)`: Resolve and validate command executable path before subprocess invocation [`src/shell_scripts/utils.py`]
      - `opencode.run(...)`: Launch OpenCode CLI in project context via command token `opencode` [`src/shell_scripts/commands/opencode.py`]
        - `require_project_root(...)`: Enforce git-root context or terminate process [`src/shell_scripts/utils.py`]
          - `get_project_root(...)`: Resolve git top-level directory by invoking git command [`src/shell_scripts/utils.py`]
        - `require_commands(...)`: Resolve and validate command executable path before subprocess invocation [`src/shell_scripts/utils.py`]
      - `pi.run(...)`: Launch pi.dev CLI in project context via resolved executable path and forward all CLI arguments unchanged without implicit `--tools` injection [`src/shell_scripts/commands/pi.py`]
        - `require_project_root(...)`: Enforce git-root context or terminate process [`src/shell_scripts/utils.py`]
          - `get_project_root(...)`: Resolve git top-level directory by invoking git command [`src/shell_scripts/utils.py`]
        - `require_commands(...)`: Resolve and validate command executable path before subprocess invocation [`src/shell_scripts/utils.py`]
      ...
      - `req_cmd.run(...)`: Resolve selector-driven targets, skip non-git-root targets for current/`--dirs` modes with explicit skip evidence, apply cleanup/scaffold on eligible targets, execute external `req`, and propagate external non-zero exit codes [`src/shell_scripts/commands/req_cmd.py`]
        ...
        - `req_cmd._iter_descendant_dirs(...)`: Resolve non-hidden descendant targets for `--recursive` mode [`src/shell_scripts/commands/req_cmd.py`]
          - `req_cmd._is_hidden_path(...)`: Filter hidden-path segments relative to current base directory [`src/shell_scripts/commands/req_cmd.py`]
        - `req_cmd._is_git_repository_root(...)`: Check whether each current/first-level target is the exact git repository root [`src/shell_scripts/commands/req_cmd.py`]
        - `req_cmd._print_install_skipped(...)`: Emit one deterministic skip evidence line for each non-git-root skipped target [`src/shell_scripts/commands/req_cmd.py`]
        ...
        - `req_cmd._build_req_args(...)`: Build external `req` argv with hardcoded base flags, runtime-configured provider/static-check vectors, and command-level omission of deprecated `pi:prompts` provider flags [`src/shell_scripts/commands/req_cmd.py`]
          - `get_req_profile(...)`: Resolve `req.providers` and `req.static_checks` runtime profile values with fallback to hardcoded defaults that exclude deprecated `pi:prompts` emission [`src/shell_scripts/config.py`]
