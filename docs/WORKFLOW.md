...
- `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`) and mandatory mutually exclusive exposure selector (`--ev` or `--auto-ev`), detecting source DNG bits-per-color and deriving EV ceiling `MAX=((bits_per_color-8)/2)` with fixed `0.25` granularity for static and adaptive exposure paths, printing detected bits-per-color for both selectors and printing bit-derived `MAX` when `--auto-ev` is selected, then applying shared postprocess encoding with optional auto-brightness pre-stage and optional auto-adjust implementation selector dispatch (`ImageMagick` or `OpenCV`) with shared knob sets (`--ab-*`, `--aa-*`), source DNG EXIF propagation with orientation preservation, EXIF thumbnail refresh after JPG save, and EXIF-datetime-based filesystem timestamp synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, mutually exclusive exposure selectors (`--ev` and `--auto-ev`), static EV values constrained to `0.25` granularity with DNG-bit-derived upper-bound validation deferred to runtime RAW metadata stage, raw gamma selector, mandatory backend selector, postprocess factors, JPEG compression, optional auto-brightness selector (`--auto-brightness`) with auto-brightness knobs (`--ab-*`), optional auto-adjust implementation selector (`--auto-adjust <ImageMagick|OpenCV>`) with shared auto-adjust knobs (`--aa-*`), and luminance backend options including default `flat` hdr-weight, default `mantiuk08` tone mapping, and explicit `--tmo*` passthrough selectors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._derive_supported_ev_values(...)`: Build supported `0.25`-step EV selector tuple from detected source DNG bits-per-color [`src/shell_scripts/commands/dng2hdr2jpg.py`]
