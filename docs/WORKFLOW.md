...
- Internal Call-Trace Tree:
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with selectable backend (`enfuse` default, optional `luminance-hdr-cli`) [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, EV selector, gamma selector, and luminance backend options [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._load_image_dependencies(...)`: Resolve optional Python image-processing modules (`rawpy`, `imageio`) [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._run_luminance_hdr_cli(...)`: Merge bracket TIFF images into final JPG through luminance-hdr-cli using `MTB` alignment, selected tone mapper, and explicit EV list (`-e`) for EXIF-less intermediates [`src/shell_scripts/commands/dng2hdr2jpg.py`]
