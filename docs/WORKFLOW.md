...
- `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`) and mandatory mutually exclusive exposure selector (`--ev` or `--auto-ev`), where `--auto-ev` computes adaptive EV from RAW linear-preview luminance percentiles before extraction, then applies shared postprocess encoding, optional auto-adjust implementation selector dispatch (`ImageMagick` or `OpenCV`) with one shared `--aa-*` knob set, source DNG EXIF propagation with orientation preservation, EXIF thumbnail refresh after JPG save, and EXIF-datetime-based filesystem timestamp synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, mutually exclusive exposure selectors (`--ev` and `--auto-ev`), raw gamma selector, mandatory backend selector, postprocess factors, JPEG compression, optional auto-adjust implementation selector (`--auto-adjust <ImageMagick|OpenCV>`), shared auto-adjust knobs (`--aa-*`), and luminance backend options including default `flat` hdr-weight, default `reinhard02` tone mapping, and explicit `--tmo*` passthrough selectors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._encode_jpg(...)`: Convert merged HDR TIFF payload into final JPG output with shared gamma/brightness/contrast/saturation processing, optional auto-adjust-stage dispatch by selected implementation mode using one shared auto-adjust knob payload, RGBA-to-RGB normalization, source EXIF payload copy, and post-save EXIF thumbnail refresh [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._resolve_auto_adjust_opencv_dependencies(...)`: Resolve OpenCV auto-adjust Python dependencies (`cv2`, `numpy`) used by OpenCV auto-adjust-stage execution [`src/shell_scripts/commands/dng2hdr2jpg.py`]
