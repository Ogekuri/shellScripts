    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`) and mandatory mutually exclusive exposure selector (`--ev` or `--auto-ev`), where `--auto-ev` computes adaptive EV from RAW linear-preview luminance percentiles before extraction, then applies shared postprocess encoding, optional auto-adjust implementation selector dispatch (`ImageMagick` or `OpenCV`), source DNG EXIF propagation with orientation preservation, EXIF thumbnail refresh after JPG save, and EXIF-datetime-based filesystem timestamp synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._encode_jpg(...)`: Convert merged HDR TIFF payload into final JPG output with shared gamma/brightness/contrast/saturation processing, optional auto-adjust-stage dispatch by selected implementation mode, RGBA-to-RGB normalization, source EXIF payload copy, and post-save EXIF thumbnail refresh [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._apply_validated_auto_adjust_pipeline(...)`: Execute validated ImageMagick auto-adjust sequence over temporary lossless 16-bit artifacts before final JPG encoding [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._apply_validated_auto_adjust_pipeline_opencv(...)`: Execute validated OpenCV auto-adjust sequence with deterministic uint8-to-uint16 upconversion, explicit uint16-to-float normalization, staged float processing, and float-to-uint16 restoration before final JPG encoding [`src/shell_scripts/commands/dng2hdr2jpg.py`]
