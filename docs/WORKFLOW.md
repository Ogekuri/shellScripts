...
- Internal Call-Trace Tree:
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`), then execute in-memory 16-bit postprocess, optional in-memory 16-bit adaptive `magic_retouch`, and final JPG encoding [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, EV selector, raw gamma selector, mandatory backend selector, postprocess factors, JPEG compression, optional adaptive `--magic-retouch` controls (`--magic-denoise-threshold`, `--magic-gamma-bias`, `--magic-clahe-clip-limit`, `--magic-vibrance-strength`, `--magic-sharpen-strength`, `--magic-sharpen-threshold`), reject legacy and removed filter-based magic options, and parse luminance backend options including default `flat` hdr-weight, default `reinhard02` tone mapping, and explicit `--tmo*` passthrough selectors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._magic_retouch(...)`: Execute optional deterministic adaptive OpenCV stage (noise estimation + denoise gate, luminance-aware gamma, optional CLAHE, adaptive vibrance, edge-masked unsharp sharpening) after postprocess and before JPG conversion, using neutral defaults [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._to_float01_from_u16(...)`: Normalize uint16 payload to float working domain [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._to_u16_from_float01(...)`: Convert normalized float payload back to uint16 [`src/shell_scripts/commands/dng2hdr2jpg.py`]
