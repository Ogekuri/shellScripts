...
- Internal Call-Trace Tree:
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2hdr2jpg.run(...)`: Execute shared DNG-to-HDR pipeline and finalize output as JPG [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        - `dng2hdr2jpg._run_shared_dng2hdr_command(...)`: Shared runtime for JPG/TIFF command flows with common parsing, backend merge, postprocess, and optional magic-retouch [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, EV selector, raw gamma selector, backend selector, shared postprocess factors, optional JPG compression, optional adaptive `--magic-retouch` controls, reject removed legacy controls, and parse luminance backend options including explicit `--tmo*` passthrough [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            ...
            - `dng2hdr2jpg._parse_non_negative_float_option(...)`: Validate non-negative magic-retouch float options [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            - `dng2hdr2jpg._parse_unit_float_option(...)`: Validate unit-interval magic-retouch float options [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            - `dng2hdr2jpg._parse_positive_int_option(...)`: Validate positive integer option values [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            - `dng2hdr2jpg._parse_magic_gamma_bias_option(...)`: Validate `--magic-gamma-bias` in bounded signed range [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._load_image_dependencies(...)`: Resolve optional Python image-processing modules (`rawpy`, `imageio`, `Pillow`, `opencv-python`, `numpy`) [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._read_u16_image(...)`: Read merged HDR TIFF payload and normalize it into in-memory uint16 RGB stage input [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._apply_postprocess_16bit(...)`: Apply in-memory 16-bit `post-gamma`, `brightness`, `contrast`, and `saturation` before final encoding [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._magic_retouch(...)`: Execute optional deterministic adaptive OpenCV stage (parameterized denoise by `denoise_strength`, luminance-aware gamma, optional CLAHE, adaptive vibrance, edge-masked unsharp sharpening) after postprocess and before final encoding, using neutral defaults [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            - `dng2hdr2jpg._to_float01_from_u16(...)`: Normalize uint16 payload to float working domain [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            - `dng2hdr2jpg._to_u16_from_float01(...)`: Convert normalized float payload back to uint16 [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._encode_final_jpg(...)`: Adapt shared payload to JPG final-encoder callback shape [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            - `dng2hdr2jpg._encode_jpg(...)`: Convert in-memory uint16 RGB payload into final JPG output using ordered-dither uint16->uint8 quantization and artifact-reduction JPEG flags (`optimize`, `progressive`, `subsampling=0`) with configured compression mapping [`src/shell_scripts/commands/dng2hdr2jpg.py`]
              - `dng2hdr2jpg._u16_to_u8_ordered_dither(...)`: Apply deterministic ordered dithering before uint8 quantization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
      ...
      - `dng2hdr2tiff.run(...)`: Execute shared DNG-to-HDR pipeline and finalize output as lossless 16-bit TIFF [`src/shell_scripts/commands/dng2hdr2tiff.py`]
        - `dng2hdr2jpg.run_tiff(...)`: Bind TIFF command to shared command runner with TIFF-specific settings [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._run_shared_dng2hdr_command(...)`: Shared runtime for JPG/TIFF command flows with common parsing, backend merge, postprocess, and optional magic-retouch [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._encode_final_tiff(...)`: Adapt shared payload to TIFF final-encoder callback shape [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            - `dng2hdr2jpg._encode_tiff(...)`: Persist final uint16 RGB payload as lossless TIFF without quantization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
