...
- `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`), mandatory mutually exclusive exposure selector (`--ev` or `--auto-ev`), optional EV-center selectors (`--ev-zero` or `--auto-zero`), optional percentage scalers (`--auto-zero-pct`, `--auto-ev-pct`, default `50`), source DNG bits-per-color detection, bit-derived `BASE_MAX=((bits_per_color-8)/2)`, safe EV-center ceiling `SAFE_ZERO_MAX=(BASE_MAX-1)`, and `MAX_BRACKET=(BASE_MAX-abs(ev_zero))` resolution, fixed `0.25`-step static/adaptive exposure paths, and zero-centered luminance merge EV list (`-ev_delta,0,+ev_delta`), then shared postprocess encoding with optional auto-brightness pre-stage and optional auto-adjust implementation selector dispatch (`ImageMagick` or `OpenCV`) with shared knob sets (`--ab-*`, `--aa-*`), source DNG EXIF propagation with orientation preservation, EXIF thumbnail refresh after JPG save, and EXIF-datetime-based filesystem timestamp synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, mutually exclusive exposure selectors (`--ev` and `--auto-ev`), optional manual EV-center selector (`--ev-zero`) and optional automatic EV-center selector (`--auto-zero`) with mutual-exclusion enforcement, optional percentage scalers (`--auto-zero-pct`, `--auto-ev-pct`) constrained to `0..100` with default `50`, static EV values constrained to `0.25` granularity with DNG-bit-derived upper-bound validation deferred to runtime RAW metadata stage, raw gamma selector, mandatory backend selector, postprocess factors, JPEG compression, optional auto-brightness selector (`--auto-brightness`) with auto-brightness knobs (`--ab-*`), optional auto-adjust implementation selector (`--auto-adjust <ImageMagick|OpenCV>`) with shared auto-adjust knobs (`--aa-*`), and luminance backend options including default `flat` hdr-weight, default `mantiuk08` tone mapping, and explicit `--tmo*` passthrough selectors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._derive_supported_ev_values(...)`: Build supported `0.25`-step bracket EV selector tuple from detected source DNG bits-per-color and selected `ev_zero` center [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._derive_supported_ev_zero_values(...)`: Build supported non-negative EV-zero magnitudes constrained by `SAFE_ZERO_MAX=(BASE_MAX-1)` to preserve at least `±1EV` bracket [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._detect_dng_bits_per_color(...)`: Detect source DNG bits-per-color by preferring RAW sample-container bit depth from `raw_image_visible.dtype.itemsize*8` and falling back to white-level scalar/sequence payload when container metadata is unavailable [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._parse_auto_zero_option(...)`: Validate `--auto-zero` assignment activator token for automatic EV-center enablement [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._parse_percentage_option(...)`: Validate percentage option values for `--auto-zero-pct` and `--auto-ev-pct` within inclusive `0..100` range [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._parse_ev_zero_option(...)`: Validate `--ev-zero` value granularity with deferred bit-derived bound validation at runtime RAW metadata stage [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        - `dng2hdr2jpg._resolve_ev_zero(...)`: Resolve effective EV-center from manual selector or automatic median-based auto-zero computation, then apply percentage scaling with downward quarter-step quantization toward zero before safe-range clamp [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._optimize_auto_zero(...)`: Compute scene-key-preserving EV-center candidate from preview median target class, clamp to bit-derived base range, and quantize to quarter-step grid [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            - `dng2hdr2jpg._derive_scene_key_preserving_median_target(...)`: Derive low-key/mid-key/high-key median target used by auto-zero EV-center optimization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        - `dng2hdr2jpg._resolve_ev_value(...)`: Resolve effective EV selector by choosing static `--ev` value or computing adaptive EV when `--auto-ev` is selected, then apply percentage scaling with downward quarter-step quantization toward zero and clamp to supported range [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._extract_normalized_preview_luminance_stats(...)`: Decode linear RAW preview and extract normalized low/median/high luminance percentiles [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._compute_auto_ev_value(...)`: Decode linear RAW preview, compute normalized luminance percentiles, and optimize adaptive EV selector [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            - `dng2hdr2jpg._compute_auto_ev_value_from_stats(...)`: Compute adaptive EV selector from pre-extracted normalized luminance percentiles [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            ...
            - `dng2hdr2jpg._optimize_adaptive_ev_delta(...)`: Compute constrained symmetric adaptive EV candidate around resolved EV-center from percentile-based shadow/highlight terms [`src/shell_scripts/commands/dng2hdr2jpg.py`]
              ...
              - `dng2hdr2jpg._quantize_ev_toward_zero_step(...)`: Quantize signed EV values toward zero on `0.25` grid for deterministic downward scaling behavior [`src/shell_scripts/commands/dng2hdr2jpg.py`]
              - `dng2hdr2jpg._apply_auto_percentage_scaling(...)`: Apply percentage-based EV scaling and perform downward quarter-step quantization toward zero [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._resolve_default_postprocess(...)`: Resolve backend- and TMO-dependent postprocess default factors before final option materialization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._build_exposure_multipliers(...)`: Compute centered bracket multipliers `2^(ev_zero-ev_delta)`, `2^(ev_zero)`, and `2^(ev_zero+ev_delta)` from static or adaptive EV selector and selected `ev_zero` center [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._run_luminance_hdr_cli(...)`: Merge bracket TIFF images into intermediate HDR TIFF through luminance-hdr-cli using zero-reference EV list (`-e <-ev_delta,0,+ev_delta>`), base hdrModel/hdrWeight/hdrResponseCurve/tmo controls, mandatory `--ldrTiff 16b`, and explicit `--tmo*` passthrough parameters, executing backend from output-TIFF parent directory to isolate backend sidecar artifacts inside temporary-workspace lifecycle [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._encode_jpg(...)`: Convert merged HDR TIFF payload into final JPG output with optional scene-key-preserving auto-brightness pre-stage executed in 16-bit domain with luminance-only gain mapping before uint8 JPG conversion, static gamma/brightness/contrast/saturation processing without EV-center compensation gain, optional auto-adjust-stage dispatch by selected implementation mode using shared auto-adjust knob payload, RGBA-to-RGB normalization, source EXIF payload copy, and post-save EXIF thumbnail refresh [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._to_uint8_image_array(...)`: Normalize image tensors to uint8 for JPG/postprocess stages after optional auto-brightness and for OpenCV auto-adjust dispatch [`src/shell_scripts/commands/dng2hdr2jpg.py`]
- `dng2hdr2jpg._to_uint16_image_array(...)`: Normalize image tensors to uint16 for auto-brightness pre-stage execution in 16-bit domain [`src/shell_scripts/commands/dng2hdr2jpg.py`]
- `dng2hdr2jpg._clip_histogram_autolevel_channel(...)`: Execute luminance-channel histogram clipping and linear clamp-only autolevel rescaling for auto-brightness pre-stage on uint8/uint16 luminance channels [`src/shell_scripts/commands/dng2hdr2jpg.py`]
- `dng2hdr2jpg._apply_mean_gamma_correction_channel(...)`: Execute luminance-channel gamma correction driven by current/target mean using native channel range (`uint8`/`uint16`) for auto-brightness pre-stage [`src/shell_scripts/commands/dng2hdr2jpg.py`]
- `dng2hdr2jpg._derive_scene_key_preserving_auto_brightness_target(...)`: Derive low-key/mid-key/high-key gamma target for auto-brightness luminance correction while preserving scene-key classification [`src/shell_scripts/commands/dng2hdr2jpg.py`]
- `dng2hdr2jpg._apply_auto_brightness_rgb_uint8(...)`: Execute auto-brightness pre-stage pipeline (`histogram-clip autolevel -> CLAHE -> conditional gamma`) on 16-bit luminance derived from RGB, then apply equal per-pixel gain `g=L'/L` to RGB with anti-clipping cap to preserve chromatic neutrality before static postprocess factors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
