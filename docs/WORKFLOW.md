...
- `dng2hdr2jpg._parse_float_exclusive_range_option(...)`: Validate exclusive floating-point range constraints used by `--ab-target-grey` parsing [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._parse_auto_brightness_options(...)`: Validate and materialize auto-brightness parameters with default `target-grey=0.18` and optional `--ab-target-grey` override [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._to_linear_srgb(...)`: Convert normalized sRGB channels to linear-sRGB using IEC 61966-2-1 piecewise transfer inversion [`src/shell_scripts/commands/dng2hdr2jpg.py`]
- `dng2hdr2jpg._from_linear_srgb(...)`: Convert linear-sRGB channels to sRGB using IEC 61966-2-1 piecewise transfer function [`src/shell_scripts/commands/dng2hdr2jpg.py`]
- `dng2hdr2jpg._compute_bt709_luminance(...)`: Compute per-pixel BT.709 linear luminance `0.2126R+0.7152G+0.0722B` [`src/shell_scripts/commands/dng2hdr2jpg.py`]
- `dng2hdr2jpg._apply_highlight_rolloff(...)`: Apply global Reinhard-style highlight rolloff to linear RGB when post-gain highlights exceed unity [`src/shell_scripts/commands/dng2hdr2jpg.py`]
- `dng2hdr2jpg._apply_auto_brightness_rgb_uint8(...)`: Execute BT.709 linear auto-brightness pipeline `uint16 normalize -> linearize -> luminance p50/p98 -> raw_gain(target-grey/p50) -> light_ev(clamp(log2(raw_gain)*0.35,0,+0.5)) -> gain(min(2^light_ev,4.0,max(0.95,0.90/max(p98,1e-7)))) -> highlight rolloff -> inverse transfer -> uint16` preserving chromatic neutrality via identical channel gain application [`src/shell_scripts/commands/dng2hdr2jpg.py`]
