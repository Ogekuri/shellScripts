...
- Internal Call-Trace Tree:
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`) and mandatory mutually exclusive exposure selector (`--ev` or `--auto-ev`), where `--auto-ev` computes adaptive EV from RAW linear-preview luminance percentiles before extraction, then applies shared postprocess encoding, optional wow implementation selector dispatch (`ImageMagick`, `OpenCV`, or `OpenCV-NP`), source DNG EXIF propagation with orientation preservation, EXIF thumbnail refresh after JPG save, and EXIF-datetime-based filesystem timestamp synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, mutually exclusive exposure selectors (`--ev` and `--auto-ev`), raw gamma selector, mandatory backend selector, postprocess factors, JPEG compression, optional wow implementation selector (`--wow <ImageMagick|OpenCV|OpenCV-NP>`), and luminance backend options including default `flat` hdr-weight, default `reinhard02` tone mapping, and explicit `--tmo*` passthrough selectors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._resolve_ev_value(...)`: Resolve effective EV selector by choosing static `--ev` value or computing adaptive EV when `--auto-ev` is selected [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._parse_wow_mode_option(...)`: Validate wow implementation selector value and normalize to canonical dispatch mode (`ImageMagick` or `OpenCV`) [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._encode_jpg(...)`: Convert merged HDR TIFF payload into final JPG output with shared gamma/brightness/contrast/saturation processing, optional wow-stage dispatch by selected implementation mode, RGBA-to-RGB normalization, source EXIF payload copy, and post-save EXIF thumbnail refresh [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._resolve_wow_opencv_np_dependencies(...)`: Resolve OpenCV-NP wow Python dependencies (`cv2`, `numpy`) used by OpenCV-NP wow-stage execution [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._apply_validated_wow_pipeline_opencv_np(...)`: Execute validated OpenCV-NP wow sequence with deterministic uint16-to-float normalization, selective blur contrast gating loops, fixed-level normalization, LUT-based sigmoidal contrast, HSL saturation gamma, high-pass gray, overlay composition, and float-to-uint16 restoration before final JPG encoding [`src/shell_scripts/commands/dng2hdr2jpg.py`]
