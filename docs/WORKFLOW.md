...
- `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`), mandatory mutually exclusive exposure selector (`--ev` or `--auto-ev`), and optional `--ev-zero` center selector, detecting source DNG bits-per-color and deriving both `BASE_MAX=((bits_per_color-8)/2)` and `MAX_BRACKET=(BASE_MAX-abs(ev_zero))` with fixed `0.25` granularity for static and adaptive exposure paths, printing detected bits-per-color for both selectors and printing bit-derived ceilings when `--auto-ev` is selected, then applying shared postprocess encoding with optional auto-brightness pre-stage and optional auto-adjust implementation selector dispatch (`ImageMagick` or `OpenCV`) with shared knob sets (`--ab-*`, `--aa-*`), source DNG EXIF propagation with orientation preservation, EXIF thumbnail refresh after JPG save, and EXIF-datetime-based filesystem timestamp synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, mutually exclusive exposure selectors (`--ev` and `--auto-ev`), optional `--ev-zero` selector constrained to `0.25` granularity with DNG-bit-derived bound validation deferred to runtime RAW metadata stage, static EV values constrained to `0.25` granularity with DNG-bit-derived upper-bound validation deferred to runtime RAW metadata stage, raw gamma selector, mandatory backend selector, postprocess factors, JPEG compression, optional auto-brightness selector (`--auto-brightness`) with auto-brightness knobs (`--ab-*`), optional auto-adjust implementation selector (`--auto-adjust <ImageMagick|OpenCV>`) with shared auto-adjust knobs (`--aa-*`), and luminance backend options including default `flat` hdr-weight, default `mantiuk08` tone mapping, and explicit `--tmo*` passthrough selectors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._resolve_default_postprocess(...)`: Resolve backend- and TMO-dependent postprocess default factors before final option materialization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._run_luminance_hdr_cli(...)`: Merge bracket TIFF images into intermediate HDR TIFF through luminance-hdr-cli using explicit centered EV list (`-e <ev_zero-ev_delta,ev_zero,ev_zero+ev_delta>`), base hdrModel/hdrWeight/hdrResponseCurve/tmo controls, mandatory `--ldrTiff 16b`, and explicit `--tmo*` passthrough parameters [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._encode_jpg(...)`: Convert merged HDR TIFF payload into final JPG output with optional auto-brightness pre-stage, `ev_zero` compensation gain `2^(ev_zero)` before static gamma/brightness/contrast/saturation processing, optional auto-adjust-stage dispatch by selected implementation mode using shared auto-adjust knob payload, RGBA-to-RGB normalization, source EXIF payload copy, and post-save EXIF thumbnail refresh [`src/shell_scripts/commands/dng2hdr2jpg.py`]
