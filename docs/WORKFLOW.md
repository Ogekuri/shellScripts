  ...
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with selectable backend (`enfuse` default, optional `luminance-hdr-cli`) producing intermediate 16-bit TIFF and shared postprocess encoding [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, EV selector, raw gamma selector, postprocess factors, JPEG compression, and simplified luminance backend options [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._run_luminance_hdr_cli(...)`: Merge bracket TIFF images into intermediate HDR TIFF through luminance-hdr-cli using explicit EV list (`-e`) plus hdrModel/hdrWeight/hdrResponseCurve/tmo and mantiuk08 controls [`src/shell_scripts/commands/dng2hdr2jpg.py`]
