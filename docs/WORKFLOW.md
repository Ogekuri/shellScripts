  ...
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`), shared postprocess encoding, optional wow implementation selector dispatch (`ImageMagick`, `OpenCV`, or `OpenCV-NP`), source DNG EXIF propagation into JPG, and EXIF-datetime-based filesystem timestamp synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._extract_dng_exif_payload_and_timestamp(...)`: Read source DNG EXIF payload and resolve preferred EXIF datetime (`DateTimeOriginal` > `DateTimeDigitized` > `DateTime`) for output metadata synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
