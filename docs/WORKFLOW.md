...
- Internal Call-Trace Tree:
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2jpg.run(...)`: Convert one DNG file to one JPG via three-bracket HDR merge [`src/shell_scripts/commands/dng2jpg.py`]
        - `dng2jpg._parse_run_options(...)`: Parse positional input/output paths and optional EV selector [`src/shell_scripts/commands/dng2jpg.py`]
          - `dng2jpg._parse_ev_option(...)`: Validate EV value against supported bracket set [`src/shell_scripts/commands/dng2jpg.py`]
        - `dng2jpg._load_image_dependencies(...)`: Resolve optional Python image-processing modules (`rawpy`, `imageio`) [`src/shell_scripts/commands/dng2jpg.py`]
        - `dng2jpg._collect_processing_errors(...)`: Build handled processing exception class tuple from runtime module capabilities [`src/shell_scripts/commands/dng2jpg.py`]
        - `dng2jpg._build_exposure_multipliers(...)`: Compute `2^(-ev)`, `1.0`, and `2^(ev)` bracket multipliers [`src/shell_scripts/commands/dng2jpg.py`]
        - `dng2jpg._write_bracket_images(...)`: Generate three temporary 16-bit TIFF exposures from one RAW input [`src/shell_scripts/commands/dng2jpg.py`]
        - `dng2jpg._run_enfuse(...)`: Merge temporary bracket images into one HDR TIFF artifact [`src/shell_scripts/commands/dng2jpg.py`]
        - `dng2jpg._encode_jpg(...)`: Convert merged HDR TIFF payload into final JPG output [`src/shell_scripts/commands/dng2jpg.py`]
