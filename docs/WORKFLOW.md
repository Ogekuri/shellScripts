...
- Internal Call-Trace Tree:
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`) producing intermediate 16-bit TIFF and shared postprocess encoding [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, EV selector, raw gamma selector, mandatory backend selector, postprocess factors, JPEG compression, and luminance backend options including default `flat` hdr-weight, default `reinhard02` tone mapping, and explicit `--tmo*` passthrough selectors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._load_image_dependencies(...)`: Resolve optional Python image-processing modules (`rawpy`, `imageio`, `Pillow`) [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._encode_jpg(...)`: Convert merged HDR TIFF payload into final JPG output with shared gamma/brightness/contrast/saturation processing and RGBA-to-RGB normalization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
