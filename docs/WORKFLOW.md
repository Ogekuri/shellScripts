...
- `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`), mandatory mutually exclusive exposure selector (`--ev` or `--auto-ev`), optional EV-center selectors (`--ev-zero` or `--auto-zero`), source DNG bits-per-color detection, bit-derived `BASE_MAX=((bits_per_color-8)/2)`, safe EV-center ceiling `SAFE_ZERO_MAX=(BASE_MAX-1)`, and `MAX_BRACKET=(BASE_MAX-abs(ev_zero))` resolution, fixed `0.25`-step static/adaptive exposure paths, and zero-centered luminance merge EV list (`-ev_delta,0,+ev_delta`), then shared postprocess encoding with optional auto-brightness pre-stage and optional auto-adjust implementation selector dispatch (`ImageMagick` or `OpenCV`) with shared knob sets (`--ab-*`, `--aa-*`), source DNG EXIF propagation with orientation preservation, EXIF thumbnail refresh after JPG save, and EXIF-datetime-based filesystem timestamp synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, mutually exclusive exposure selectors (`--ev` and `--auto-ev`), optional manual EV-center selector (`--ev-zero`) and optional automatic EV-center selector (`--auto-zero`) with mutual-exclusion enforcement, static EV values constrained to `0.25` granularity with DNG-bit-derived upper-bound validation deferred to runtime RAW metadata stage, raw gamma selector, mandatory backend selector, postprocess factors, JPEG compression, optional auto-brightness selector (`--auto-brightness`) with auto-brightness knobs (`--ab-*`), optional auto-adjust implementation selector (`--auto-adjust <ImageMagick|OpenCV>`) with shared auto-adjust knobs (`--aa-*`), and luminance backend options including default `flat` hdr-weight, default `mantiuk08` tone mapping, and explicit `--tmo*` passthrough selectors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          ...
          - `dng2hdr2jpg._extract_raw_white_level_int(...)`: Normalize RAW white-level metadata to one validated positive integer scalar consumed by bit-depth and preview-normalization stages [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._detect_dng_bits_per_color(...)`: Detect source DNG bits-per-color by preferring RAW sample-container bit depth and clamping inflated container depth with white-level evidence (`<= white_level.bit_length()+2`) before fallback to white-level-derived bit depth [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._resolve_ev_zero(...)`: Resolve effective EV-center from manual selector or automatic median-based auto-zero computation [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._optimize_auto_zero(...)`: Compute median-based EV-center candidate, clamp to bit-derived base range, and quantize to quarter-step grid [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        - `dng2hdr2jpg._resolve_ev_value(...)`: Resolve effective EV selector by choosing static `--ev` value or computing adaptive EV when `--auto-ev` is selected [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._extract_normalized_preview_luminance_stats(...)`: Decode linear RAW preview and extract normalized low/median/high luminance percentiles using capped normalization reference `min(preview_max_luminance, raw_white_level)` [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._compute_auto_ev_value(...)`: Decode linear RAW preview, compute normalized luminance percentiles, and optimize adaptive EV selector [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            ...
            - `dng2hdr2jpg._optimize_adaptive_ev_delta(...)`: Compute constrained symmetric adaptive EV candidate around resolved EV-center from percentile-based shadow/highlight terms [`src/shell_scripts/commands/dng2hdr2jpg.py`]
              ...
              - `dng2hdr2jpg._quantize_ev_to_supported(...)`: Quantize clamped adaptive EV value to nearest DNG-bit-derived supported `0.25`-step bracket selector with lower-value tie resolution for midpoint floating residue [`src/shell_scripts/commands/dng2hdr2jpg.py`]
