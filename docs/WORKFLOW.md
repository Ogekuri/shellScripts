...
- `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`) and mandatory mutually exclusive exposure selector (`--ev` or `--auto-ev`), where `--auto-ev` computes adaptive EV from RAW linear-preview luminance percentiles before extraction, clamps to `0.25..3.0`, quantizes to nearest `0.25`, then applies shared postprocess encoding with optional auto-brightness pre-stage and optional auto-adjust implementation selector dispatch (`ImageMagick` or `OpenCV`) with shared knob sets (`--ab-*`, `--aa-*`), source DNG EXIF propagation with orientation preservation, EXIF thumbnail refresh after JPG save, and EXIF-datetime-based filesystem timestamp synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
...
- `dng2hdr2jpg._parse_run_options(...)`: Parse positional input/output paths, mutually exclusive exposure selectors (`--ev` and `--auto-ev`), static EV values in `0.25..3.0` with `0.25` step, raw gamma selector, mandatory backend selector, postprocess factors, JPEG compression, optional auto-brightness selector (`--auto-brightness`) with auto-brightness knobs (`--ab-*`), optional auto-adjust implementation selector (`--auto-adjust <ImageMagick|OpenCV>`) with shared auto-adjust knobs (`--aa-*`), and luminance backend options including default `flat` hdr-weight, default `mantiuk08` tone mapping, and explicit `--tmo*` passthrough selectors [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._parse_ev_option(...)`: Validate EV value against supported `0.25..3.0` bracket set with `0.25` step [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._resolve_ev_value(...)`: Resolve effective EV selector by choosing static `--ev` value or computing adaptive EV when `--auto-ev` is selected [`src/shell_scripts/commands/dng2hdr2jpg.py`]
          - `dng2hdr2jpg._compute_auto_ev_value(...)`: Decode linear RAW preview, compute luminance percentiles, and optimize adaptive EV selector [`src/shell_scripts/commands/dng2hdr2jpg.py`]
            ...
            - `dng2hdr2jpg._optimize_adaptive_ev_delta(...)`: Compute constrained adaptive EV candidate from percentile-based shadow/highlight and median-centered terms [`src/shell_scripts/commands/dng2hdr2jpg.py`]
              ...
              - `dng2hdr2jpg._quantize_ev_to_supported(...)`: Quantize clamped adaptive EV value to nearest supported `0.25`-step bracket selector [`src/shell_scripts/commands/dng2hdr2jpg.py`]
