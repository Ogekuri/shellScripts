  ...
  - `main(...)`: Entry dispatcher for all CLI invocations [`src/shell_scripts/core.py`]
    ...
    - `get_command(...)`: Resolve first positional argument to command module [`src/shell_scripts/commands/__init__.py`]
      ...
      - `dng2hdr2jpg.run(...)`: Convert one DNG file to one JPG via Linux-gated three-bracket HDR merge with mandatory mutually exclusive backend selector (`--enable-enfuse` or `--enable-luminance`), shared postprocess encoding, optional wow implementation selector dispatch (`ImageMagick`, `OpenCV`, or `OpenCV-NP`), source DNG EXIF propagation with orientation preservation, EXIF thumbnail refresh after JPG save, and EXIF-datetime-based filesystem timestamp synchronization [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._parse_exif_datetime_to_timestamp(...)`: Parse EXIF datetime scalar (`YYYY:MM:DD HH:MM:SS`) into POSIX timestamp [`src/shell_scripts/commands/dng2hdr2jpg.py`]
        ...
        - `dng2hdr2jpg._set_output_file_timestamps(...)`: Apply EXIF-derived POSIX timestamp to output JPG filesystem atime/mtime via `os.utime`, preserving capture-date synchronization even when EXIF datetime scalar is null-terminated [`src/shell_scripts/commands/dng2hdr2jpg.py`]
